			...
			<div class="bbuyer_head_nav">
				<ul>
					<?php foreach ($app_rows as $app_row): ?>
						...
						<li><a href="<?=$app_row['app_url']?>" target="_blank"><?=$app_row['app_name']?></a></li>
						<?php endif; ?>
					<?php endforeach; ?>
					<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Login&met=logout"><?= _('退出') ?></a></li>
				</ul>
			</div>
		</div>
		<script type="text/javascript">
				...
				<div class="left  ">
					...
					<div class="font">
						<table>
							<tbody>
							<tr>
								<td colspan="2" class="name"><?= $this->user['info']['user_name']; ?></td>
							</tr>
							<tr>
								<td class="fontColor"><?= _('会员级别:') ?></td>
					...
					<div class="b_rol">
						<div class=" same">
							<div class="same_1">
								...
								<a href="<?= Yf_Registry::get('url') ?>?ctl=Property&met=cash"><?= _('余额') ?></a></div>
							<div class="same_2" id="mons"></div>
						</div>
						<div class="same">
							<div class="same_1">
								<i class="iconfont  icon-iconjifen ts_2"></i>
								<a href="<?= Yf_Registry::get('url') ?>?ctl=Points&met=points"><?= _('积分') ?></a></div>
							<div class="same_2"><?= $this->user['points']['user_points']; ?></div>
						</div>
						<div class="same">
							<div class="same_1">
								<i class="iconfont icon-iconquan  ts"></i>
								<a href="<?= Yf_Registry::get('url') ?>?ctl=Voucher&met=voucher"><?= _('代金卷') ?></a></div>
				...
				<div class="left_1   visible-lg">
					...
					foreach ($buyer_menu[$level_row[1]]['sub'] as $menu_row)
					{
						...
						if (!empty($menu_row['sub']))
						{
							...
							{
								?>
								<li><a href="<?= sprintf('%s?ctl=%s&met=%s', Yf_Registry::get('url'), $menus_row['menu_url_ctl'], $menus_row['menu_url_met'], $menus_row['menu_url_parem']); ?>"
									   class="_Color" style="<?= ($menus_row['menu_id'] == $level_row[1]) ? 'color:red;' : '' ?>"><?= $menus_row['menu_name'] ?></a></li>
								<?php
							}
							?>
				...
				foreach ($buyer_menu[$level_row[1]]['sub'] as $menu_row)
				{
					...
					if (!empty($menu_row['sub']))
					{
						foreach ($menu_row['sub'] as $menus_row)
						{
							...
							?>
							<li><a href="<?= sprintf('%s?ctl=%s&met=%s', Yf_Registry::get('url'), $menus_row['menu_url_ctl'], $menus_row['menu_url_met'], $menus_row['menu_url_parem']); ?>"
								   class="_Color" style="<?= ($menus_row['menu_id'] == $level_row[3]) ? 'color:red;' : '' ?>"><?= $menus_row['menu_name'] ?></a></li>
							<?php
						}
						?>
				...
				<div class="member_infor_content">
					<div class="tabmenu">
						<ul class="tab pngFix">
							...
							{
								?>
								<li class="<?= ($menu_row['menu_id'] == $level_row[4]) ? 'active' : 'normal' ?>"><a
										href="<?= sprintf('%s?ctl=%s&met=%s', Yf_Registry::get('url'), $menu_row['menu_url_ctl'], $menu_row['menu_url_met'], $menu_row['menu_url_parem']); ?>"><?= $menu_row['menu_name'] ?></a>
								</li>
								<?php
							}
							?>
