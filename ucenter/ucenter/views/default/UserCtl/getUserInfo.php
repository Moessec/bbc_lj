			...
			<?php if($data['user_email']){?>
				...
				<?php }?>
                      ...
                      <?=_('修改手机')?></a>
						...
						<?php }?>
							<dl>
								<dt><?=_('真实姓名：')?></dt>
								<dd><span class="w400">
                        <input type="text" class="text" maxlength="20" name="user_truename" value="<?=$data['user_truename']?>">
                        </span><span class="midisplay">
                        <select name="privacy[user_privacy_realname]">
                          ...
                          <option value="2" <?php if($privacy ['user_privacy_realname'] == 2){?>selected="selected"<?php }?>><?=_('保密')?></option>
							...
							<dl>
								...
								<dd><span class="w400">
                        <label>
                          <input type="radio" name="user_gender" value="0" <?=($data['user_gender']==0 ? 'checked':'');?>>
                          <?=_('女')?></label>
                        &nbsp;&nbsp;
                        <label>
                          <input type="radio" name="user_gender" value="1" <?=($data['user_gender']==1 ? 'checked':'');?>>
                          <?=_('男')?></label>
                        &nbsp;&nbsp;
                        <label>
                          <input type="radio" name="user_gender" value="2" <?=($data['user_gender']==2 ? 'checked':'');?>>
                          <?=_('保密')?></label>
                        </span><span class="midisplay">
						...
						<label><?=_('日')?></label>
					   <b class="kbs"><?=_('填生日有惊喜哦~')?></b></span><span class="midisplay">
					...
					<?php if(@$data['user_area']){ ?>
						<div id="d_1"><?=@$data['user_area'] ?>&nbsp;&nbsp;<a href="javascript:sd();"><?=_('编辑')?></a></div>
					<?php } ?>
					<div id="d_2"  class="<?php if(@$data['user_area']) echo 'hidden';?>">
                        ...
                        <select name="privacy[user_privacy_qq]">
                          <option value="0" <?php if($privacy ['user_privacy_qq'] == 0){?>selected="selected"<?php }?>><?=_('公开')?></option>
                          <option value="1" <?php if($privacy ['user_privacy_qq'] == 1){?>selected="selected"<?php }?>><?=_('好友可见')?></option>
                          <option value="2" <?php if($privacy ['user_privacy_qq'] == 2){?>selected="selected"<?php }?>><?=_('保密')?></option>
                        </select>
                        </span> </dd>
							</dl>
							<dl class="bottom">
					...
					$.ajax({
						...
						success: function (a) {
							if (a.status == 200) {
								...
								location.href = SITE_URL + "?ctl=User&met=getUserInfo";
							} else {
