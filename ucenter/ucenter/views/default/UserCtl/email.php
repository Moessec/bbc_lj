    ...
    <div class="form-style">
		...
		<input type="hidden" value="email_verify" name="act">
            ...
            <dl>
                ...
                <input type="button" class="send" data-type="email" value="<?=_('获取邮件验证码')?>" />
                </dd>
            </dl>
            <dl class="foot">
                <dt>&nbsp;</dt>
...
$(".send").click(function(){
	...
	if(!val){
		obj.addClass('red');
	 	$("<label class='error'>"+icon+"<?=_('请填写邮箱')?></label>").insertAfter(obj);
	}
	else if(!patrn.test(val)){  
		obj.addClass('red');
		$("<label class='error'>"+icon+"<?=_('请填写正确的邮箱')?></label>").insertAfter(obj);
	}
	else{
		...
		$.post(url, pars, function (data)
		{
			if(data && 200 == data.status){
				...
				var url = SITE_URL +'?ctl=User&met=getEmailYzm&typ=json';
				var sj = new Date();
				...
				obj.addClass('red');
				$("<label class='error'>"+icon+"<?=_('该邮箱已绑定了账号')?></label>").insertAfter(obj);
			}
		});
	}
});
