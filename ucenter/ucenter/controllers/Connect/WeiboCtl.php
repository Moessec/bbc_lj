...
class Connect_WeiboCtl extends Yf_AppController implements Connect_Interface
{
	...
	public function __construct(&$ctl, $met, $typ)
	{
		...
		$this->redirect_url = sprintf('%s?ctl=Connect_Weibo&met=callback&from=%s&callback=%s', Yf_Registry::get('url'),request_string('from'), request_string('callback'));
	}
	...
	public function login()
	{
		...
		if ("Weibo")
		{
			$url = "https://api.weibo.com/oauth2/authorize?client_id=".$this->appid."&redirect_uri=".urlencode($redirect_url);
		}
		else
		{
