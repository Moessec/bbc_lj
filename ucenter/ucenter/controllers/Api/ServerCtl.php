		...
		else
		{
			...
			else
			{
				...
				if (!$check_db_rows)
				{
					...
					//
					$sql_path =  ROOT_PATH . '/../ucenter_admin/install/data/sql/';
...
EOF;
						...
						//写入配置文件
						$db_config_file            = ROOT_PATH . '/../ucenter_admin/ucenter_admin/configs/db_' . $server_id . '.ini.php';
						$db_config_row             = array();
						$db_config_row['host']     = $db_host;
						$db_config_row['port']     = 3306;
						$db_config_row['user']     = $db_user;
						...
						//ucenter
						$ucenter_config_file = ROOT_PATH . '/../ucenter_admin/ucenter_admin/configs/ucenter_api_' . $server_id . '.ini.php';
						file_put_contents($ucenter_config_file, $ucenter_config_contents);

						//paycenter
						$paycenter_config_file = ROOT_PATH . '/../ucenter_admin/ucenter_admin/configs/paycenter_api_' . $server_id . '.ini.php';
						file_put_contents($paycenter_config_file, $paycenter_config_contents);

						//shop
						$shop_config_file = ROOT_PATH . '/../ucenter_admin/ucenter_admin/configs/shop_api_' . $server_id . '.ini.php';
						file_put_contents($shop_config_file, $shop_config_contents);
