...
class Order_SettlementModel extends Order_Settlement
{
	...
	//实物订单结算
	public function settleNormalOrder()
	{
		//1.查找店铺的结算周期
		...
		foreach ($shop_info as $key => $val)
		{
			...
			if ($time > $end_unixtime)
			{
				...
				//应结金额（订单金额（含运费）-佣金金额-退单金额+退还佣金-店铺费用+定金订单中的未退定金+下单时使用的平台红包-全部退款时应扣除的平台红包）
				$add_settle_row['os_amount'] = $settle_row['order_amount'] - $settle_row['commission_amount'] - $settle_row['return_amount'] + $settle_row['commission_return_amount'] - $settle_row['shop_cost'];
				//生成结算单时间
	...
	//结算虚拟订单
	public function settleVirtualOrder()
	{
		//1.查找店铺的结算周期
		$Shop_BaseModel = new Shop_BaseModel();
		$shop_info      = $Shop_BaseModel->getSettlementCycle();
