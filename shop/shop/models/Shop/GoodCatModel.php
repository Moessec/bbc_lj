...
class Shop_GoodCatModel extends Shop_GoodCat
{
	...
	public function getGoodCatList($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
	{
		...
		$data                  = $this->getByWhere($cond_row, $order_row, $page, $rows);

		$shop_goods_cat_id = array_column($data, 'shop_goods_cat_id');

		$shop_goods_cat_id['parent_id:IN'] = $shop_goods_cat_id;
		$subclass_rows                     = $this->getByWhere($shop_goods_cat_id);

		foreach ($subclass_rows as $keys => $subclass_row)
		{
			$subclass_row['shop_goods_cat_statuscha'] = _(self::$shop_goods_cat_status[$subclass_row['shop_goods_cat_status']]);

			$data[$subclass_row['parent_id']]['subclass'][] = $subclass_row;

			//$data[$subclass_row['parent_id']]['shop_goods_cat_statuscha'] = _(self::$shop_goods_cat_status[$data[$subclass_row['parent_id']]['shop_goods_cat_status']]);
		}
		foreach ($data as $key => $val)
		{
			$data[$key]['shop_goods_cat_statuscha'] = _(self::$shop_goods_cat_status[$val['shop_goods_cat_status']]);
		}
