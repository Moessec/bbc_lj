	...
	*/
	...
	public function getGroupBuyInfoByGoodsCommonID($common_id)
	{
		...
		if (Web_ConfigModel::value('groupbuy_allow'))  //团购开启
		{
			...
			$row                        = $this->GroupBuy_BaseModel->getGroupBuyDetailByWhere($cond_row);
			if (!empty($row))
			{
				if ($row['groupbuy_state'] == GroupBuy_BaseModel::NORMAL)
				{
					$renew_row = $row;
				}
			}
	...
	public function getXianShiGoodsInfoByGoodsID($goods_id)
	{
		...
		if (Web_ConfigModel::value('promotion_allow')) //商品促销开启，包括限时折扣，满送，加价购
		{
			...
			$row                              = $this->Discount_GoodsModel->getDiscountGoodsDetailByWhere($cond_row);

			if (!empty($row))
			{
				if ($row['discount_goods_state'] == Discount_GoodsModel::NORMAL)
				{
					$renew_row = $row;
				}
			}
