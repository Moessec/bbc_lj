...
class ExpressModel extends Express
{
	...
	public function getExpressList($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
	{
		...
		foreach ($data["items"] as $key => $value)
		{
			...
			$data["items"][$key]["express_status"]       = _(ExpressModel::$expressStatus[$value["express_status"]]);
			$data["items"][$key]["express_displayorder"] = _(ExpressModel::$expressDisplayorder[$value["express_displayorder"]]);
			$data["items"][$key]["express_commonorder"]  = _(ExpressModel::$expressCommonorder[$value["express_commonorder"]]);
