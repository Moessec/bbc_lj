...
?>
    <script type='text/jade' id='thrid_opt'>
				<a class="bbc_seller_btns button add button_blue" href="<?= Yf_Registry::get('index_page') ?>?ctl=Seller_Goods&met=addformat&typ=e&act=add"><i class="iconfont icon-jia"></i>添加新版式</a>
    ...
    <div class="search fn-clear">
        ...
        <form id="search_form" class="search_form_reset" method="get" action="<?= Yf_Registry::get('url') ?>">
            <input class="text w150" type="text" name="key" value="<?= ($key ? $key : ''); ?>" placeholder="请输入版式名称"/>
...
if (!empty($data))
{
    ...
    <form id="form" method="post" action="index.php?ctl=Seller_Goods&met=editGoodsCommon">
        <table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
            <tr>
                <th class="tl">
                    <label class="checkbox"><input class="checkall" type="checkbox"/></label>板式名称
                </th>
                <th width="80">位置</th>
                <th width="120">操作</th>
            </tr>
            ...
            foreach ($data as $key => $value)
            {
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span class="edit"><a
                                href="<?php echo Yf_Registry::get('url'); ?>?ctl=Seller_Goods&met=addformat&act=edit&id=<?= $value['id']; ?>"><i
