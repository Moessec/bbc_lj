    ...
    <?php
     ...
     ?>
        <li>
            ...
            <div class="goods-btn">
                <div data-type="btn_add_act_goods"  data-id="<?=$goods['goods_id']?>"  btn-enabled="<?=$goods['goods_id']?>"  class="<?=$goods['is_join']=='true'?'hidden':'button'?> button_green"><i class="iconfont icon-jia"></i><?=_('设置为活动商品')?></div>
                <div data-id="<?=$goods['goods_id']?>" href="javascript:void(0);" btn-disabled="<?=$goods['goods_id']?>" class="<?=$goods['is_join']=='true'?'':'hidden'?> "><?=_('已加入活动')?></div>
            </div>
        </li>
            <?php 	}	?>
    </ul>
