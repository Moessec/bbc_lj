...
<style>
    ...
    <div class="alert alert-block">
        <h4><?=_('说明')?>：</h4>
        <ul>
            <li><?=_('满即送活动包括店铺所有商品，活动时间不能和已有活动重叠')?></li>
            <li><?=_('每个满即送活动最多可以设置3个价格级别，点击新增级别按钮可以增加新的级别，价格级别应该由低到高')?></li>
            <li><?=_('每个级别可以有减现金、送礼品2种促销方式，至少需要选择一种')?></li>
        </ul>
    </div>
        ...
        <form method="post" id="form" action="<?=Yf_Registry::get('url')?>?ctl=Seller_Promotion_MeetConditionGift&met=add&typ=e">
            ...
            <dl>
                ...
                <dd>
                    ...
                    <p class="hint"><?=_('开始时间不能为空且不能小于')?><?=$data['combo']['combo_start_time']?></p>
                </dd>
            </dl>
            <dl>
                <dt><i>*</i><?=_('结束时间')?>：</dt>
                <dd>
                    ...
                    <p class="hint"><?=_('结束时间不能为空')?><?php if(!$data['shop_type']){ ?><?=_('且不能晚于')?><?=$data['combo']['combo_end_time']?><?php } ?></p>
                </dd>
            </dl>
            ...
            <dl>
                ...
                <dd>
                    ...
                    <a href="javascript:void(0);" id="add_rule_btn" class="button button_blue bbc_seller_btns"><i class="iconfont icon-jia"></i><?=_('添加规则')?></a>
                    <div id="add_rule_box" style="display:none;">
                        <div class="ncsc-mansong-error error">
                            <span id="mansong_price_error" style="display:none;">
                                <i class="iconfont icon-exclamation-sign"></i><?=_('规则金额不能为空且必须为数字')?>
                            </span>
                            <span id="mansong_discount_error" style="display:none;">
                                <i class="iconfont icon-exclamation-sign"></i><?=_('满减金额必须小于规则金额')?>
                            </span>
                        </div>
                        <div class="ncsc-mansong-rule">
                            <span><?=_('单笔订单满')?>&nbsp;<input id="mansong_price" type="text" class="text w50"><em><?=Web_ConfigModel::value('monetary_unit')?></em>，</span>
                            <span><?=_('立减现金')?>&nbsp;<input id="mansong_discount" type="text" class="text w50"><em><?=Web_ConfigModel::value('monetary_unit')?></em>，</span>
                            <span><?=_('送礼品')?>&nbsp;<a href="javascript:void(0);" id="btn_show_search_goods" class="button button_orange bbc_seller_btns"><i class="icon-gift"></i><?=_('选择礼品')?></a></span>
                            ...
                            <div class="search-goods-list fn-clear" style="display: none;">
                                <div class="search-goods-list-hd">
                                    <label><strong><?=_('第一步：搜索店内商品')?></strong></label>
                                    <input id="search_goods_name" type="text w150" class="text" name="goods_name" value=""/>
                                    <a href="javascript:void(0);"  class="button btn_search_goods"/><i class="iconfont icon-btnsearch"></i><?=_('搜索')?></a>
                                    <span class="hint" style="margin-left:10px;"><?=_('不输入名称直接搜索将显示店内所有出售中的商品')?></span>
                                </div>
                                <a id="btn_hide_search_goods" class="close" href="javascript:void(0);">X</a>
                                <div class="search-goods-list-bd fn-clear"></div>
                            </div>
                        </div>
                        <div id="mansong_rule_error" style="display:none;"><?=_('请至少选择一种促销方式')?></div>
                        <div class="mt10">
                            <a href="javascript:void(0);" id="btn_save_rule" class="button bbc_seller_btns"><i class="icon-ok-circle"></i><?=_('确定规则设置')?></a>
                            <a href="javascript:void(0);" id="btn_cancel_add_rule" class="button bbc_seller_btns"><i class="icon-ban-circle"></i><?=_('取消')?></a>
                        </div>
                    </div>
                    <span class="error-message"></span>
                    <p class="hint"><?=_('设置当单笔订单满足金额时（必填选项），减免金额（选填）或赠送的礼品（选填）；留空为不做减免金额或赠送礼品处理')?>。</p>
                    <p class="hint"><?=_('系统最多支持设置三组等级规则')?>。</p>
                </dd>
            </dl>
            ...
            <dl>
                ...
                <dd>
                    ...
                    <p class="hint"><?=_('活动备注最多为100个字符')?></p>
                </dd>
            </dl>
...
<script id="mansong_rule_template" type="text/html">
    <li nctype="mansong_rule_item" class="mansong_rule_item">
        <span><?=_('单笔订单满')?><strong><?=Web_ConfigModel::value('monetary_unit')?><%=price%></strong>， </span>
        <span><?=_('立减现金')?><strong><?=Web_ConfigModel::value('monetary_unit')?><%=discount%></strong> </span>
        <%if(goods_id>0){%>
        <span>，<?=_('送礼品')?> <%==goods%></span>
        <%}%>
        <input type="hidden" name="mansong_rule[]" value="<%=price%>,<%=discount%>,<%=goods_id%>">
        <a nctype="btn_del_mansong_rule" href="javascript:void(0);" class="mini-btn bbc_seller_btns"><i class="iconfont icon-lajitong"></i><?=_('删除')?></a>
    </li>
</script>
...
<script id="mansong_goods_template" type="text/html">
    ...
    </div><a nctype="btn_del_mansong_goods" href="javascript:void(0);" class="mini-btn bg-red"><i class="iconfont icon-trash"></i><?=_('删除已选择的礼品')?></a>
</script>
