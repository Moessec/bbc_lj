...
<div class="exchange">
	<div class="alert">
        <?php if($shop_type){ ?>
            <ul>
                <li>1、<?=_('手工设置代金券失效后,用户将不能领取该代金券,但是已经领取的代金券仍然可以使用')?></li>
                <li>2、<?=_('代金券模版和已发放的代金券过期后自动失效')?></li>
            </ul>
        <?php  }else{ ?>
		<h4>
            <?php if($com_flag){ ?><?=_('套餐过期时间')?>：<em class="red"></em><?=$combo['combo_end_time']?>。
            <?php }else{ ?>
	...
	<table class="table-list-style table-promotion-list" id="table_list" width="100%" cellpadding="0" cellspacing="0">
		<tr>
			<th class="tc" width="150"><?=_('代金券名称')?></th>
			<th width="100"><?=_('消费金额')?></th>
			<th width="60"><?=_('面额')?></th>
            <th width="100"><?=_('会员级别')?></th>
			<th width="80"><?=_('领取方式')?></th>
			<th width="60"><?=_('状态')?></th>
			<th width="200"><?=_('有效期')?></th>
        ...
        <tr class="row_line">
            ...
            <td class="nscs-table-handle">
				<span class="edit"><a href="<?=Yf_Registry::get('url')?>?ctl=Seller_Promotion_Voucher&met=add&op=edit&id=<?=$value['voucher_t_id']?>&typ=e"><i class="iconfont icon-zhifutijiao"></i><?=_('编辑')?></a></span>
				<span class="edit del_line"><a href="<?=Yf_Registry::get('url')?>?ctl=Seller_Promotion_Voucher&met=index&op=detail&id=<?=$value['voucher_t_id']?>&typ=e"><i class="iconfont icon-btnclassify2"></i><?=_('详情')?></a></span>
                <span class="del"><a data-param="{'ctl':'Seller_Promotion_Voucher','met':'removeVoucherTemp','id':'<?=@$value['voucher_t_id']?>'}" href="javascript:void(0)"><i class="iconfont icon-lajitong"></i><?=_('删除')?></a></span>
            </td>
        </tr>
        <?php } }else{ ?>
            <tr class="row_line">
