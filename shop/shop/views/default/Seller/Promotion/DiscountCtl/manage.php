        ...
        if($data['discount_goods_rows'])
        {
            foreach($data['discount_goods_rows'] as $key=>$value)
            {
                ...
                <tr class="row_line">
                    <td width="50">
                        <div class="pic-thumb">
                            <a href="<?=Yf_Registry::get('url')?>?ctl=Goods_Goods&met=goods&gid=<?=$value['goods_id']?>&typ=e" target="_blank">
                                <img alt="" data-src="<?=image_thumb($value['goods_image'],36,36)?>" src="<?=$value['goods_image']?>" style="max-width:36px;max-height:36px;"/>
                            </a>
                        </div>
                    </td>
                    <td class="tl"><a target="_blank" href="<?=Yf_Registry::get('url')?>?ctl=Goods_Goods&met=goods&gid=<?=$value['goods_id']?>&typ=e"><?=$value['goods_name']?></a></td>
                    ...
                    <td>
                        <span class="edit"><a href="javascript:void(0);" optype="btn_edit_discount_goods" data-discount-goods-id="<?=$value['discount_goods_id']?>" data-goods-price-format = "<?=format_money($value['goods_price'])?>" data-goods-price="<?=$value['goods_price']?>" ><i class="iconfont icon-zhifutijiao"></i><?=_('编辑')?></a></span>
...
<div id="dialog_edit_discount_goods" class="eject_con" style="display:none;">
    ...
    <dl>
		...
		<p id="dialog_edit_discount_goods_error" style="display:none;"><label for="dialog_edit_discount_goods_error" class="error"><i class='icon-exclamation-sign'></i><?=_('折扣价格不能为空，且必须小于商品价格')?></label></p>
    </dl>
...
<table style="display:none;">
	<tbody id="table_list_template">
		<tr class="row_line">
			...
			<td>
				<span class="edit"><a href="javascript:void(0);" optype="btn_edit_discount_goods" data-discount-goods-id="__discountgoodsid" data-goods-price-format = "__goodspriceformat" data-goods-price="__goodsprice" ><i class="iconfont icon-zhifutijiao"></i><?=_('编辑')?></a></span>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.search-goods-list-bd').on('click', '[data-type="btn_add_goods"]', function() {
           ...
           /* $('.eject_con').addClass('dialog_content');
            $.dialog({
                title: "<?=_('限时折扣商品规则设定')?>",
                content: $('#dialog_add_discount_goods').html(),
                //data: data,
                width: 450,
                //height: 440,
                max: !1,
        ...
        $('.search-goods-list-bd').on('click', '#btn_submit', function() {
            ...
            var goodspriceformat = discount_goods_param.goodspriceformat = Number($('#dialog_input_goods_price_format').val());
