...
<div class="freight">
	<div class="tabmenu">
		<ul>
        	<li ><a href="<?=Yf_Registry::get('url')?>?ctl=Seller_Transport&met=transport"><?=_('运费设置')?></a></li>

            <?php if($act == 'add') {?>
            <li  class="active bbc_seller_bg"><a href="javascript:void(0);"><?=_('添加运费模版')?></a></li>
            <?php }
            if($act == 'edit'){?>
            <li class="active bbc_seller_bg"><a href="javascript:void(0);"><?=_('编辑运费模版')?></a></li>
            <?php }?>
        </ul>
    ...
    <input type="hidden" name="default" id="default" value=""/>
        ...
        <dl class="dl">
            <dt><i><?=_('*')?></i><?=_('首重重量：')?></dt>
            <dd style="width:25%"><input type="text" class="text w60" name="default_num" id="default_num" value="" /><em><?=_('Kg')?></em></dd>
            <dt style="width:8%"><i><?=_('*')?></i><?=_('首重费用：')?></dt>
            <dd><input type="text" class="text w60" name="default_price" id="default_price" value="" /><em><?=(Web_ConfigModel::value('monetary_unit'))?></em></dd>
        </dl>
        <dl class="dl">
            <dt><i><?=_('*')?></i><?=_('续重重量：')?></dt>
            <dd style="width:25%"><input type="text" class="text w60" name="add_num" id="add_num" value="" /><em><?=_('Kg')?></em></dd>
            <dt style="width:8%"><i><?=_('*')?></i><?=_('续重费用：')?></dt>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#form').validator({
            ...
            fields: {
                'type_name':'required',
                'default_num': 'required;range[0~1]',
                'default_price': 'required;range[0~100]',
                'add_num':'range[0~10]',
                'add_price':'range[0~100]',
                'city[]':'checked',
            },
            valid:function(form){
                //表单验证通过，提交表单
