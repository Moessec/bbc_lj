...
<div class="tabmenu">
    <ul>
        <li class="active bbc_seller_bg"><a href="javascript:void(0);"><?=_('投诉管理')?></a></li>
    </ul>
</div>
    <div class="search fn-clear">
        <form id="search_form" method="get">
            ...
            <input type="text" name="keys" class="text w200" placeholder="<?=_('请输入投诉内容')?>" value="<?=$data['keys']?>"/>
            <select name="status" style="margin-right: 10px;">
                <option value=""><?=_('请选择状态')?></option>
                <option value="1" <?php if($data['state'] == 1){echo "selected='selected'";}?>>新投诉</option>
                <option value="2" <?php if($data['state'] == 2){echo "selected='selected'";}?>>待申诉</option>
                <option value="3" <?php if($data['state'] == 3){echo "selected='selected'";}?>>对话中</option>
                <option value="4" <?php if($data['state'] == 4){echo "selected='selected'";}?>>待仲裁</option>
                <option value="5" <?php if($data['state'] == 5){echo "selected='selected'";}?>>已关闭</option>
            </select>
...
<table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
    ...
    foreach ($data['items'] as $key => $value)
    {
        ...
        <tr>
            ...
            <td width="450" style="text-align: left;"><a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?= $value['good']['goods_id'] ?>"><?= $value['good']['goods_name'] ?></a></td>
            <td><?= $value['complain_subject_content'] ?></td>
            <td><?= $value['complain_datetime'] ?></td>
            <td><?= $value['complain_state_text'] ?></td>
            <td>
                <span><a href="./index.php?ctl=Seller_Service_Complain&met=index&act=detail&id=<?=$value['complain_id']?>"><i class="iconfont icon-chakan"></i><?=_('查看')?></a></span>
            </td>
        </tr>
    <?php } ?>
    <?php }else{ ?>
