...
<div class="tabmenu">
    <ul>
        ...
        } ?>><a href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Service_Consult&met=index"><?=_('全部咨询')?></a></li>
        <li <?php if ($data['state'] == 1)
        {
            echo 'class="active bbc_seller_bg"';
        } ?>><a href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Service_Consult&met=index&status=1"><?=_('未回复咨询')?></a></li>
        <li <?php if ($data['state'] == 2)
        {
            echo 'class="active bbc_seller_bg"';
        } ?>><a href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Service_Consult&met=index&status=2"><?=_('已回复咨询')?></a></li>
    </ul>
</div>
...
<form method="post" id="form" action="./index.php?ctl=Seller_Service_Consult&met=delAllConsult&typ=json">
    <table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <th class="tl">
                <label class="checkbox"><input class="checkall" type="checkbox"></label><?=_('咨询/回复')?>
            </th>
        ...
        foreach ($data['items'] as $key => $value)
        {
            ...
            <tr>
                <td class="tl" colspan="2">
                    ...
                    <a target="_blank"
                       href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?= $value['goods_id'] ?>"><?= $value['goods_name'] ?> </a>
                </td>
            </tr>
            <tr>
                <td class="tl" style="padding-left: 38px;">
                    <p>咨询内容：<?= $value['consult_question'] ?></p>
                    <?php if($value['consult_answer']){ ?><p><?=_('回复：')?><?= $value['consult_answer'] ?>(<?= $value['answer_time'] ?>)</p><?php }?>
                </td>
                <td>
                    <span class="edit"><a href="javascript:void(0)" data-id="<?= $value['consult_id'] ?>"><i
                                class="iconfont icon-jieshi"></i><?php if($value['consult_answer']){ echo _('修改');}?><?=_('回复')?></a></span>
                    <span class="del"><a
                            data-param="{'ctl':'Seller_Service_Consult','met':'delConsult','id':'<?= $value['consult_id'] ?>'}"
                            href="javascript:void(0)"><i class="iconfont icon-lajitong"></i><?=_('删除')?></a></span>
                </td>
            </tr>
        <?php } ?>
        <td class="toolBar" colspan="99" <?php if(!$page_nav){echo "style='border-bottom:none;'";}?>>
            ...
            <span>|</span>
            <label class="del" data-param="{'ctl':'Seller_Service_Consult','met':'delAllConsult'}"><i class="iconfont icon-lajitong"></i><?=_('删除')?></label>
            <div class="page">
                <?=$page_nav?>
            </div>
        </td>
