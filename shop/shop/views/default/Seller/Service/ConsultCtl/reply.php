    ...
    <div id="warning" class="alert alert-error"></div>
            <form id="form" action="#" method="post">
                ...
                <dl>
                    <dt><?=_('回复内容：')?></dt>
                    <dd>
                        <textarea name="consult_answer" class="textarea_text"><?=$data['consult_answer']?></textarea>
                    </dd>
                </dl>
                <div class="bottom">
                    <label class="submit-border"><input id="handle_submit" class="submit bbc_seller_btns" value="<?=_('回复')?>"/></label>
                </div>
            </form>
</div>
    ...
    $(document).ready(function ()
    {
        $('#form').validator({
            ...
            valid: function (form)
            {
                //表单验证通过，提交表单
                $.ajax({
                    url: SITE_URL + '?ctl=Seller_Service_Consult&met=replyConsult&typ=json',
                    data: $("#form").serialize(),
                    success: function (a)
                    {
                        if (a.status == 200)
                        {
                            window.parent.location.href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Service_Consult&met=index&status=2";
                            $.dialog().close();
                        }
                        else
                        {
