...
<div class="tabmenu">
    <ul>
        <li class="active bbc_seller_bg"><a href="javascript:void(0);"><?=_('退款管理')?></a></li>
    </ul>
</div>
...
<table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
    ...
    foreach ($data['items'] as $key => $value)
    {
        ...
        <tr>
            <td>
                <p style="text-align: left;"><?=_('订单编号:')?><?= $value['order_number'] ?></p>
                <p style="text-align: left;" class="getit"><?=_('退款编号:')?><?= $value['return_code'] ?></p>
            </td>
            ...
            <td>
                    <span><a
                            href="./index.php?ctl=Seller_Service_Return&met=orderReturn&act=detail&id=<?= $value['order_return_id'] ?>"><i class="iconfont icon-chakan"></i><?=_('查看')?></a></span>
            </td>
        </tr>
    <?php } ?>
    <?php if($data['page']){ ?>
