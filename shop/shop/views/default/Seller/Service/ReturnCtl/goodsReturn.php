...
<div class="tabmenu">
    <ul>
        <li class="active bbc_seller_bg"><a href="javascript:void(0);"><?=_('退货管理')?></a></li>
    </ul>
</div>
<div class="search fn-clear">
    <form id="search_form" method="get">
        ...
        <input type="text" name="keys" class="text w200" placeholder="<?=_('请输入商品名称')?>" value="<?= $data['keys'] ?>"/>
...
<table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
    ...
    foreach ($data['items'] as $key => $value)
    {
        ...
        <tr>
            <td>
                <dl class="fn-clear">
                    <dd class="alr">
                        ...
                        <h3><a style="text-overflow: ellipsis;width:350px;height: 20px;display: inline-block;overflow: hidden;line-height: 26px;white-space: nowrap;" href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?= $value['good']['goods_id'] ?>" target="_blank" title="<?= $value['order_goods_name'] ?>"><?= $value['order_goods_name'] ?></a></h3>
                        <p style="padding-top: 3px;"><?=_('订单编号:')?><?= $value['order_number'] ?></p>
                        <p style="padding-top: 3px;" class="getit"><?=_('退款编号:')?><?= $value['return_code'] ?></p>
                            </div>
                    </dd>
                </dl>
            </td>
            ...
            <td>
                    <span><a
                            href="./index.php?ctl=Seller_Service_Return&met=goodsReturn&act=detail&id=<?= $value['order_return_id'] ?>"><i class="iconfont icon-chakan"></i><?=_('查看')?></a></span>
            </td>
        </tr>
    <?php } ?>
    <?php if($data['page']){ ?>
