...
?>
    ...
    <dl class="bill-hd fn-clear">
        ...
        <dd>
        	<!--<a class="button button_red" href="">本期结算无误，我要确认</a>-->
	...
	<div class="tabmenu">
		<ul>
        	<li class="<?php if($type == 'active'): ?>active<?php endif;?>"><a href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Order_Settlement&met=virtual&op=show&id=<?=($id)?>&type=active"><?=_('订单列表')?></a></li>
        	<li class="<?php if($type == 'refund'): ?>active<?php endif;?>"><a href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Order_Settlement&met=virtual&op=show&id=<?=($id)?>&type=refund"><?=_('退款订单')?></a></li>
        	<li class="<?php if($type == 'cost'): ?>active<?php endif;?>"><a href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Order_Settlement&met=virtual&op=show&id=<?=($id)?>&type=cost"><?=_('促销活动')?></a></li>
        </ul>
    </div>
	<?php if($type == 'returd'):?>
	<table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
            ...
            <td><span><a href="<?= Yf_Registry::get('url') ?>?ctl=Seller_Trade_Order&met=virtualInfo&o&typ=e&order_id=<?=$val['order_id'] ?>"><i class="iconfont icon-btnclassify2"></i><?=_('查看')?></a></span></td>
        </tr>
        <?php endforeach; ?>
		<?php else: ?>
        <tr>
