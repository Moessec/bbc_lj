...
<?php include $this->view->getTplPath() . '/' . 'seller_header.php'; ?>
    ...
    <table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
		<tbody>
			...
			<tr>
			    <th class="tl" width="100"><?=_('是否默认')?></th>
				<th class="tl" width="100"><?=_('联系人')?></th>
				<th class="tl"><?=_('发货地址')?></th>
				<th width="120"><?=_('联系方式')?></th>
			...
			<tr>
				...
				<!--- 操作 --->
				<td>
					<span class="edit"><a href="<?=Yf_Registry::get('url')?>?ctl=Seller_Trade_Deliver&met=addAddress&op=edit&shipping_address_id=<?=($val['shipping_address_id'])?>"><i class="iconfont icon-zhifutijiao"></i><?=_('编辑')?></a></span>
					<span class="del"><a data-param="{'ctl':'Seller_Trade_Deliver','met':'delAddress','id':'<?= $val['shipping_address_id'] ?>'}" href="javascript:void(0)"><i class="iconfont icon-lajitong"></i><?=_('删除')?></a></span>
				</td>
			</tr>
			<?php } ?>
