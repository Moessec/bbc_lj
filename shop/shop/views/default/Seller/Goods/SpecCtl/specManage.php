    ...
    <form id="form" method="post">
        ...
        <table class="table-list-style" width="100%">
            <tbody><tr>
                ...
                <th width="110"><a id="add-sv" class="button button_blue bbc_seller_btns">添加规格值</a></th>
            </tr>
            ...
            <tr>
                ...
                <td>
            <span class="delete">
            	<a data-id="<?php echo $val['spec_value_id']; ?>"><i class="iconfont icon-lajitong"></i>删除</a>
            </span>
                </td>
            </tr>
            <?php } ?>
            ...
            <tr>
                <td colspan="3" class="foot">
                    <input type="button" id="submit-button" class="button button_red bbc_seller_btns" value="提交保存规格值">
                </td>
            </tr>
            </tbody></table>
    </form>
    ...
    $(function() {
        ...
        $('#submit-button').on('click', function (e) {
            ...
            $('input[name$="[spec_value_name]"').each(function (index, element) {
                ...
                if( element.value == '' ) {
                    ...
                    if ( !$(element).hasClass('error') ) {
                        $(element).addClass('error').after('<label class="error"><i class="iconfont icon-exclamation-sign"></i>请填写内容</label>');
                    }
                    return false;
                }
            })
            ...
            if (submitFlag) {
                ...
                $.post(SITE_URL + "?ctl=Seller_Goods_Spec&met=saveSpecValue&typ=json&spec_id=" + param_data.spec_id, $('#form').serialize(), function(data){
                    ...
                    if(data.status == 200) {
                        parent.Public.tips({ type: 3, content: '修改成功!'}), window.location.reload();
                    } else {
                        parent.Public.tips({ type: 1, content: '修改失败!'})
                    }
                })
            }
        })
        ...
        $('#add-sv').on('click', function () {
            ...
            var $addSpec = $(
                ...
                '<td><span class="delete"><a data-newId="' + newid + '"><i class="iconfont icon-lajitong"></i>删除</a></span></td>' +
                '</tr>');
