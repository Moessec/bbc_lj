...
<div class="eject_con" id="eject_con">
  <form id="form" method="post" action="#" >
      <input name="brand_id" type="hidden" value="<?=$brand_info['brand_id']?>" >
   
    <dl>
      <dt><i class="required">*</i><?=_('品牌名称：')?></dt>
      <dd>
          <input class="text w200"   style=" height: 30px;" type="text" name="brand_name" value="<?=$brand_info['brand_name']?>" />
      </dd>
    </dl>
    <dl>
      <dt><?=_('品牌类别：')?></dt>
      <input type="hidden" name="cat_id" id="cat_id" value="<?php if(!empty($brand_info['cat_id'])){echo $brand_info['cat_id'];}?>">
      <dd>
          <p id="cat_name"></p>
      </dd>
    </dl>
    <dl>
      <dt><i class="required">*</i><?=_('品牌图标：')?></dt>
     <dd>
         <div ><span class="sign"><img width="150" height="50" id="brand_img" src="<?php if(!empty($brand_info['brand_pic'])){ echo $brand_info['brand_pic'];}else{?><?=$this->view->img?>/default/default_brand_image.gif<?php }?>"></span></div>
         <input type="hidden" class="input-file" name="brand_pic" value="<?=$brand_info['brand_pic']?>"id="brand_input">

        <div id="brand_upload" style="width: 190px;margin-top: 10px" >
                <p class="lblock bbc_img_btns" ><i class="iconfont icon-tupianshangchuan"></i><?=_('图片上传')?></p>
        </div>
        <p class="hint"><?=_('建议上传大小为150x50的品牌图片。')?><br><?=_('申请品牌的目的是方便买家通过品牌索引页查找商品，申请时请填写品牌所属的类别，方便平台归类。在平台审核前，您可以编辑或撤销申请。')?></p>
      </dd>
    </dl>
    <div class="bottom">
...
<script>
	$(function() {
		...
		var opts = {
			...
			showRoot : true,
                        rootTxt: '<?php if(!empty($brand_info['cat_name'])){echo $brand_info['cat_name'];}else{    echo "选择分类";}?>',
		}
...
{ 
 ...
 $(document).ready(function(){
         var ajax_url = './index.php?ctl=Seller_Shop_Brand&met=editBrandrow&typ=json';
        $('#form').validator({
            ignore: ':hidden',
            theme: 'yellow_right',
            timely: 1,
           ...
           valid:function(form){
                ...
                $.ajax({
                    ...
                    success:function(a){
                        if(a.status == 200)
                        {
                            parent.Public.tips.success('修改成功!');
                              window.setTimeout("refreshPage()",3000);
                        }
                        else
                        {
                             parent.Public.tips.error('修改失败!');
                        }
                    }
                });
            }
