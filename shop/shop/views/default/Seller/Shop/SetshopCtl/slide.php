...
?>
 ...
 <div class="alert">
        <ul>
            <li><?=_('1、最多可上传5张幻灯片图片。')?></li>
            <li><?=_('2、支持jpg、jpeg、gif、png格式上传，建议图片宽度1200px、高度在300px到500px之间、大小1.00M以内的图片。提交2~5张图片可以进行幻灯片播放，一张图片没有幻灯片播放效果。')?></li>
            <li><?=_('3、操作完成以后，按"提交"按钮，可以在当前页面进行幻灯片展示。')?></li>
            <li><?=_('4、跳转链接必须带有')?><b style="color:red;"><?=_('"http://"')?></b></li>
        </ul>
    </div>
    ...
    <form method="post" id="form">
       ...
       <?php foreach ($array as $key => $value) {
        ...
        <table width="20%">
            ...
            <tr>
                <td>
                <a class="del button" href="javascript:del(<?=$value?>);"><?=_('删除')?></a>
                <a  id="slide_upload<?=$key?>" class="lblock bbc_img_btns"><i class="iconfont icon-tupianshangchuan"></i><?=_('图片上传')?></a>
                </td>
            </tr>
        </table>
       <?php  } ?>
        ...
        <dl class="handle_pic_foot">
            <dd><input type="submit" value="<?=_('提交')?>" class="button bbc_seller_submit_btns"></dd>
        </dl>
    </form>
