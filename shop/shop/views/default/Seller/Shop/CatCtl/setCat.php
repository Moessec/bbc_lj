...
<body>
	...
	<div class="tabmenu">
		<ul>
        	<li ><a href="./index.php?ctl=Seller_Shop_Cat&met=cat&typ=e"><?=_('店铺分类')?></a></li>

            <?php if($act == 'add') {?>
            <li  class="active bbc_seller_bg"><a href="javascript:void(0);"><?=_('添加店铺分类')?></a></li>
            <?php }
            if($act == 'edit'){?>
            <li class="active bbc_seller_bg"><a href="javascript:void(0);"><?=_('编辑店铺分类')?></a></li>
            <?php }?>
        </ul>
    ...
    <form id="form" action="#" method="post" >
        ...
        <dl class="dl">
         ...
         <dl class="dl">
            <dt><?=_('是否显示：')?></dt>
            <dd >  <label class="radio status"><input type="radio"  name="cat[shop_goods_cat_status]" id="status" checked="checked" value="1" /><?=_('是')?></label> <label class="radio status"><input type="radio"  name="cat[shop_goods_cat_status]" id="status" value="0" /><?=_('否')?></label>
        ...
        <dl class="dl">
            ...
            <dd >
                <select name="cat[parent_id]">
                    <option name="" value="0"><?=_('请选择')?></option>
                      <?php if($data) {
    ...
    <div class="form-style">
        ...
        <dl class="dl">
         ...
         <dl class="dl">
            <dt><?=_('是否显示：')?></dt>
            <dd >  <label class="radio status"><input type="radio"  <?=($data['shop_goods_cat_status'] ? 'checked' : '') ?> name="<?=$data['shop_goods_cat_status']?>" id="status"  value="1" /><?=_('是')?></label> <label class="radio status"><input type="radio"  name="<?=$data['shop_goods_cat_status']?>" <?=(!$data['shop_goods_cat_status'] ? 'checked' : '') ?> value="0" /><?=_('否')?></label></dd>
        </dl>
       <dl class="dl">
            <dt><i></i><?=_('排序：')?></dt>
