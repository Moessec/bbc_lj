...
?>
       <div class="aright">
        <div class="buyer_center_list">
          <div class="my_orders">
            <p class="my_orders_tit clearfix"><span><?=_('我的订单')?></span><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical" target="_blank"><?=_('查看全部订单')?><i class ="iconfont icon-iconjiantouyou"></i></a></p>
            <!-- 有内容显示 -->
            ...
            <table >
			  ...
			  <?php if(!empty($val['goods_list'])){?>
                <tr>
                  <td class="my_orders_goods"><a href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?=$val['goods_list'][0]['goods_id']?>" target="_blank"><img src="<?php if($val['goods_list'][0]['goods_image']){?><?=image_thumb($val['goods_list'][0]['goods_image'],50,50)?><?php }else{?><?=image_thumb($this->web['goods_image'],50,50)?><?php }?>" height="50" width="50"/></a></td>
                  ...
                  <td class="order_pay_status"><p class="wait_pay bbc_color"><?=$val['order_state_con']?></p><p><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical&act=details&order_id=<?=($val['order_id'])?>" class="pay_order_det" target="_blank"><?=_('订单详情')?></a></p></td>
                  <td><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical&act=details&order_id=<?=($val['order_id'])?>"><?=_('查看')?></a></td>
                </tr>
			  <?php } ?>
			  <?php } ?>
              </tbody>
          ...
          <div class="shop_cart_list">
            <p class="my_orders_tit clearfix"><span><?=_('购物车')?></span></span><a  href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Cart&met=cart" target="_blank"><?=_('查看所有商品')?><i class ="iconfont icon-iconjiantouyou"></i></a></p>
              ...
              <ul class="shop_cart_list_ul clearfix">
                ...
                <li>
                  <div class="clearfix">
                    ...
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?=$v['goods_id']?>" target="_blank"><?=$v['goods_base']['goods_name']?></a>
                  </div>
                  <p class="clearfix"><a class ="bbc_btns" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Cart&met=confirm&product_id=<?=$v['cart_id'];?>"><?=_('去支付')?></a></p>
                </li>
                <?php } ?>
			  <?php } ?>
              </ul>
          ...
          <div class="buyer_goods_save">
            <p class="my_orders_tit clearfix"><span><?=_('商品收藏')?></span><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Favorites&met=favoritesGoods" target="_blank"><?=_('查看更多')?><i class ="iconfont icon-iconjiantouyou"></i></a></p>
             <!-- 有内容显示 -->
			...
			<a  class="btn_left btns iconfont icon-btnreturnarrow"data-num="0" data-numb="0"></a>
              <div class="goods_save_div">
				...
				<?php if(!empty($val['detail'])){?>
                  <li>
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?=$val['detail']['goods_id']?>" target="_blank">
                      <img src="<?php if(!empty($val['detail']['goods_image'])){?><?=image_thumb($val['detail']['goods_image'],118,118)?><?php }else{?><?= image_thumb($this->web['goods_image'],118,118)?><?php }?>" height="118" width="118"/>
          ...
          <div class="buyer_exc1">
            <div class="buyer_my_track">
              <p class="my_orders_tit clearfix"><span><?=_('我的足迹')?></span><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Favorites&met=footprint" target="_blank"><?=_('查看更多')?><i class ="iconfont icon-iconjiantouyou"></i></a></p>
          ...
          <div class="buyer_exc2">
            <div class="buyer_store_collection">
              <p class="my_orders_tit clearfix"><span><?=_('店铺收藏')?></span><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Favorites&met=favoritesShop" target="_blank"><?=_('查看更多')?><i class ="iconfont icon-iconjiantouyou"></i></a></p>
