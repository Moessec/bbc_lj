...
<table class="ncm-default-table annoc_con">
    ...
    <tr class="bd-line">
		...
		<?php if($val['voucher_state_label'] == '未用'){?><span><a class="btn-grapefruit"  href="<?= Yf_Registry::get('url') ?>?ctl=Shop&met=index&id=<?=$val['voucher_shop_id']?>"><?=_('去使用')?></a></span><?php }else{?><span class="del"><a class="btn-grapefruit" data-param="{'ctl':'Buyer_Voucher','met':'delVoucher','id':'<?=$val['voucher_id']?>'}" href="javascript:void(0)"><i class="iconfont icon-lajitong"></i><?=('删除')?></a></span><?php }?>
        </td>
