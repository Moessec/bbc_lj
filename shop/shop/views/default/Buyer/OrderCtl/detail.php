          ...
          <div class="ncm-order-info">
            <div class="ncm-order-details">
              ...
              <div class="content">
                ...
                <dl class="line line2">
                  <dt><?=_('虚拟单号：')?></dt>
                  <dd><?=($data['order_id'])?><a href="#" class="ncbtn"><?=_('更多')?><i class="iconfont icon-iconjiantouxia"></i>
            ...
            <div class="ncm-order-condition">
              ...
              <ul>
                <li><?=_('1. 您尚未对该订单进行支付，请')?><a href="<?= Yf_Registry::get('paycenter_api_url') ?>?ctl=Info&met=pay&uorder=<?=($data['order_id'])?>" class="ncbtn-mini ncbtn-bittersweet bbc_btns"><i></i><?=_('支付订单')?></a><?=_('以确保及时获取电子兑换码。')?></li>
                    ...
                    <em <?php if($codeval['virtual_code_status'] != Order_GoodsVirtualCodeModel::VIRTUAL_CODE_USED):?> class="cyellowl" <?php endif; ?>>
                        <?php if($codeval['virtual_code_status'] == Order_GoodsVirtualCodeModel::VIRTUAL_CODE_NEW ):?>
                            ...
                            <!--S 判断该商品是否支持过期退款，若支持则在过期后出现退款按钮 -->
                                ...
                                <a target="_blank" style="padding: 0 3px;margin-left: 230px;" class="bbc_btns" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=add&gid=<?=($data['goods_list'][0]['order_goods_id'])?>"><?=_('过期退款')?></a>
                                <?php }?>
