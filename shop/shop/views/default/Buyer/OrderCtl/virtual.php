    ...
    $(function(){
      ...
      <div class="order_content">
          ...
          <form method="get" id="search_form" action="index.php" >
            ...
            <p class="order_types">
				<a <?php if($status == '' &&  !$recycle):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=virtual"><?=_('全部订单')?></a>
				<a <?php if($status == 'wait_pay'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=virtual&status=wait_pay"><?=_('待付款')?></a>
				<a <?php if($status == 'wait_confirm_goods'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=virtual&status=wait_confirm_goods"><?=_('待收货')?></a>
				<a <?php if($status == 'finish'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=virtual&status=finish"><?=_('已完成')?></a>
			</p>
            ...
            <p class="order_types serc_p">
                <a <?php if($recycle):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=virtual&recycle=1"><i class="iconfont icon-lajitong icon_size20"></i><?=_('订单回收站')?></a>
            </p>
          ...
          <table>
              ...
              <!-- 下单时间，订单号，店铺名称    -->
                                ...
                                <td class="td_color widt4">
                                    <?php if($val['order_status'] != Order_StateModel::ORDER_WAIT_PAY  && $val['order_status'] != Order_StateModel::ORDER_CANCEL){?>
                                        <?php if($ogval['goods_refund_status'] == Order_StateModel::ORDER_GOODS_RETURN_NO ){?>
                                            <a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=add&gid=<?=($ogval['order_goods_id'])?>"><?=_('退款')?></a>
                                        <?php }?>
                                        <?php if($ogval['goods_refund_status'] != Order_StateModel::ORDER_GOODS_RETURN_NO ){?>
                                            <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=detail&id=<?=($ogval['order_return_id'])?>"><?=_('退款进度')?></a>
                                        <?php }?>
                                     <?php }?>
                                </td>
                            </tr>
                ...
                <td class="td_rborder widt5" >
				     <span>
				        <?=_('订单总额：')?><strong><?=format_money($val['order_goods_amount'])?><!--<br/>--><?/*=($val['payment_name'])*/?>
				     </span>
				...
				<td class="td_rborder">
                   ...
                   <p>
                      <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=virtual&act=detail&order_id=<?=($val['order_id'])?>"><?=_('订单详情')?>
                      </a>
                    </p>
