...
<body>
	<div class="reset-address" >
		...
		<div class="res-cont">
			 <input type="hidden" name="user_address_id" id="id" value="<?php if(isset($data)){ echo $data['user_address_id']; }?>">
			<div class="resAdd-inp">
				<div><span><strong></strong><?=('收件人：')?></span></div>
				<input type="text"  name="user_address_contact" value="<?php if(isset($data)){echo $data['user_address_contact'];}?>">
			</div>
			<div class="resAdd-inp">
				<div><span><strong></strong><?=('手机：')?></span></div>
				<input type="text"  name="user_address_phone" value="<?php if(isset($data)){echo $data['user_address_phone'];}?>">
			</div>
			<div class="resAdd-inp">
				<div><span><strong></strong><?=('所在地区：')?></span></div>
				<input type="hidden" name="address_area" id="t" value="<?php if(isset($data)){ echo $data['user_address_area']; }?>" />
					<input type="hidden" name="province_id" id="id_1" value="<?php if(isset($data)){ echo $data['user_address_province_id']; }?>" />
					<input type="hidden" name="city_id" id="id_2" value="<?php if(isset($data)){ echo $data['user_address_city_id']; }?>" />
					<input type="hidden" name="area_id" id="id_3" value="<?php if(isset($data)){ echo $data['user_address_area_id']; }?>" />
			...
			<div class="resAdd-inp">
				<div><span><strong></strong><?=('详细地址：')?></span></div>
              	<textarea name="user_address_address" style="width:360px;height:80px;"><?php if(isset($data)){echo $data['user_address_address'];}?></textarea>
			</div>
			<div class="moren">
				<input type="checkbox" name="user_address_default" value="1" <?php if(isset($data) && $data['user_address_default'] == 1){echo 'checked';}?>><span><?=('设为默认收货地址')?></span>
			</div>
