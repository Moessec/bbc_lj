...
?>
      ...
      <div class="member_infor_content">
        <div class="tabmenu">
          <ul class="tab">
           <li class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Message&met=message"><?=_('系统消息')?>(<?=$this->countMessage['message']?>)</a></li>
			...
			<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Message&met=message&op=messageManage"><?=_('接收设置')?></a></li>
			<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Message&met=message&op=sendMessage"><?=_('发送站内信')?></a></li>
			</ul>
        </div>
<div class="message_type fn-clear">
	<a class="<?php if($type=='0'){?>cur<?php }?>" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Message&met=message"><?=_('全部类型')?></a>
...
<input type="hidden" name="type" value="<?=$type?>" />
       <!-- <h3><span></span></h3>-->
        ...
        <dl class="fn-clear">
            <dt>
				...
				<?=$val['message_create_time'];?><span class="edit"><a  class="btn-bluejeans" data-id="<?= $val['message_id'] ?>" dialog_width="550" dialog_title="<?=_('查看消息')?>"  href="javascript:void(0)"></a></span>
            </dt>
            <dd>
    ...
    <div class="toolBar fn-clear">
        <label class="checkall"><input class="checkall" type="checkbox" /> <?=_('全选')?></label>
        <span>|</span>
        <label class="del" ><i class="iconfont icon-lajitong del_pos"></i><a data-param="{'ctl':'Buyer_Message','met':'delAllMessage'}" ><?=_('删除')?></a></label>
