	...
	<div class="member_infor_content">
      <div class="tabmenu">
		<ul class="tab">
			...
			<li class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Message&met=message&op=sendMessage"><?=_('发送站内信')?></a></li>
        </ul>
    </div>
	...
	<ul>
	   ...
	   <?php foreach($data['items'] as $key=>$val){?>
        <li><a href="javascript:void(0);" id="<?=$val['friend_name']?>" nc_type="to_member_name">
          <div class="avatar"><img src="<?php if(!empty($val['friend_image'])){?><?=image_thumb($val['friend_image'],50,50)?><?php }else{?><?=image_thumb($this->web['user_logo'],50,50)?><?php }?>"></div>
          <p><?=$val['friend_name']?></p>
        </a></li>
	   <?php }?>       
	   <?php }?>       
            ...
            valid:function(form){
                ...
                $.ajax({
                    ...
                    success:function(a){
						...
						{
                            Public.tips.error("<?=_('你发送的内容涉及到敏感词！')?>");
                        }else if(a.status == 240)
						{
                            Public.tips.error("<?=_('请求用户有问题！')?>");
							
                        }else if(a.status == 260)
						{
                            Public.tips.error("<?=_('收件人不能为空！')?>");
                        }else
                        {
