      ...
      <table class="ncm-default-table annoc_con">
          <thead>
            <tr>
              ...
              <th class="w100"><?php if($op=='receive'){?><?=_('发信人')?><?php }else{?><?=_('收件人')?><?php }?></th>
              <th class="tl opti" class="w300"><?=_('内容')?></th>
              <th class="w300"><?=_('最后更新')?></th>
              <th class="w150"><?=_('操作')?></th>
            </tr>
		  ...
		  <tbody>
		   ...
		   <?php
            ...
            <tr class="bd-line">
			  ...
			  <?php if($op == 'receive'){ ?>
				<span class="edit"><a  class="btn-bluejeans" data-id="<?= $value['user_message_id'] ?>" dialog_width="550" dialog_title="<?=_('查看消息')?>"  href="javascript:void(0)"><i class="iconfont iconf icon-chakan"></i><?=_('查看')?><!--<p class="bbuyer_news"><?=$value['receive']?></p>--></a></span>
			<?php }?>
					<span class="del"><a class="btn-grapefruit" data-param="{'ctl':'Buyer_Message','met':'delUserMessage','id':'<?=$value['user_message_id']?>'}" href="javascript:void(0)"><i class="iconfont icon-lajitong"></i><?=_('删除')?></a></span></td>
            </tr>
			<?php }?>
          </tbody>
		  <?php }else{?>
