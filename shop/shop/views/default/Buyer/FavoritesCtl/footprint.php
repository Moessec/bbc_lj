...
?>
 ...
 <ul class="tracks_con_types clearfix">
			<?php if(!empty($cat)){ ?>
            <li><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Favorites&met=footprint" class="<?php if($classid == ''){ ?>bbc_btns<?php }?>"><?=_('全部分类')?></a></li>
...
<div class="tracks_con_more">
            ...
            <div class="tracks_con_time_list">
              ...
              <p class="tracks_con_time_p"><i class="bgred"></i><span></span><time><?=$ke?></time><a href="javascript:void(0)" data-param="{'ctl':'Buyer_Favorites','met':'delFootPrint','time':'<?=$ke?>','classid':'<?=$class?>'}" class="delete" title="<?=_('删除')?>"><i class="icon-trash iconfont icon-lajitong"></i><?=_('删除')?></a></p>
              <ul class="clearfix li_hover">
				<?php foreach($va as $k=>$v){ ?>
				<?php if(!empty($v['detail'])){?>
                <li>
                 <a href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?=$v['detail']['goods_id']?>" target="_blank"> <img src="<?php if($v['detail']['common_image']){?><?=image_thumb($v['detail']['common_image'],118,118)?><?php }else{?><?=image_thumb($this->web['goods_image'],118,118)?><?php }?>" height="118" width="138"/></a>
                  <span><?=format_money($v['detail']['common_price'])?></span>
                  <a class="add_cart" href="javascript:void(0)" title="<?=_('加入购物车')?>" data-param="{'ctl':'Buyer_Cart','met':'addCart','id':'<?=$v['detail']['goods_id']?>','num':'1'}"><i class="iconfont icon-zaiqigoumai"></i><?=_('加入购物车')?></a>
                </li>
				<?php }?>
				<?php }?>
