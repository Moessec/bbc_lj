...
<div class="order_content">
    ...
    <script type="text/javascript">
        ...
        <table class="complaint icos">
            <tr>
                <td colspan="2" class="w420"><p><?=_('投诉商品')?></p></td>
                <td width="143"><p><?=_('投诉主题')?></p></td>
                <td width="237"><p><?=_('投诉时间')?></p></td>
                <td width="96"><p><?=_('投诉状态')?></p></td>
                <td width="146"><p><?=_('操作')?></p></td>
            </tr>
            ...
            foreach ($data['items'] as $key => $value)
            {
                ...
                <tr>
                    ...
                    <td width="345" style="text-align: left;"><a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?= $value['good']['goods_id'] ?>"><?= $value['good']['goods_name'] ?></a></td>
                    <td><p><?= $value['complain_subject_content'] ?></p></td>
                    <td><p><?= $value['complain_datetime'] ?></p></td>
                    <td><p><?= $value['complain_state_text'] ?></p></td>
                    <td><p><span class="edit"><a href="./index.php?ctl=Buyer_Service_Complain&met=index&act=detail&id=<?= $value['complain_id'] ?>"><i class="iconfont icon-chakan"></i><?=_('查看')?></a></span><span class="cancel del_line"><a data-param="{'ctl':'Buyer_Service_Complain','met':'cancelComplain','id':'<?= $value['complain_id'] ?>'}" href="javascript:void(0)" <?php if($value['complain_state_etext']=="finish"){ ?> style="opacity: 0.3;" data-dis="1"<?php } ?>><i class="iconfont icon-lajitong"></i><?=_('取消')?></a></span></p></td>
                </tr>
            <?php } ?>
            <?php }
            else
