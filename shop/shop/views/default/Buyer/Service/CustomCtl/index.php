...
<div class="order_content">
    <div class="div_head tabmenu clearfix">
        <ul class="tab pngFix clearfix">
            <li class="active"><a><?=_('平台客服咨询列表')?></a></li>
    ...
    <div class="order_content_title clearfix">
        <div style="margin-top: 10px;" class="clearfix">
            <div class="ptkf bbc_btns">
                <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Custom&met=index&act=add&typ=e"><?=_('平台客服')?></a>
            </div>
        </div>
    </div>
        ...
        <table style="width: 100%;" class="icos bort">
            <tr>
                <td class="td_1" style="text-align: center;" width="415"><p><?=_('咨询内容')?></p></td>
                <td class="td_2" width="150"><p><?=_('咨询时间')?></p></td>
                <td class="td_3" width="100"><p><?=_('状态')?></p></td>
                <td class="td_3" width="150"><p><?=_('最后回复时间')?></p></td>
                <td class="td_4" width="150"><p><?=_('操作')?></p></td>
            </tr>
            <?php
            if (!empty($data['items']))
            {
            ...
            foreach ($data['items'] as $key => $value)
            {
                ...
                <tr>
                    ...
                    <?php } ?>
                    <td class="td_4"><p><span class="edit"><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Custom&met=index&act=detail&id=<?= $value['custom_service_id'] ?>"><i class="iconfont icon-chakan"></i><?=_('查看')?></a></span>
                            <span class="del del_line"><a
                                    data-param="{'ctl':'Buyer_Service_Custom','met':'delService','id':'<?= $value['custom_service_id'] ?>'}"
                                    href="javascript:void(0)"><i class="iconfont icon-lajitong"></i><?=_('删除')?></a></span></p></td>
                </tr>
            <?php } ?>
            <?php }
            else
