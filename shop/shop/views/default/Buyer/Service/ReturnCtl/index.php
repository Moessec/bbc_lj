    ...
    <div class="order_content">
        <div class="div_head  tabmenu clearfix">
            <ul class="tab pngFix clearfix">
                <li <?php if (empty($data['state']) || $data['state'] == 1)
                {
                    ...
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&state=1"><?= _('退款申请') ?></a>
                </li>
                <li <?php if ($data['state'] == 2)
                {
                    echo 'class="active"';
                } ?>>
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&state=2"><?= _('退货申请') ?></a>
                </li>
                <li <?php if ($data['state'] == 3)
                {
                    echo 'class="active"';
                } ?>>
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&state=3"><?= _('虚拟兑码退款') ?></a>
                </li>
            </ul>
        </div>
        ...
        <div class="order_content_title clearfix">
            <div style="margin-top: 10px;" class="clearfix">
                <form id="search_form" method="get">
                    ...
                    <p class="pright">
                        <span style="line-height: 25px;"><?= _('申请时间') ?></span> <input type="text" name="start_time" id="start_time" class="A" value="<?= $start_time ?>" placeholder="开始时间"><em class="add-on2"><i style="font-size: 22px;position: relative;left: 2px;" class="iconfont icon-rili"></i></em><em style="line-height: 25px;">&nbsp;–&nbsp;</em><input type="text" name="end_time" id="end_time" class="A" value="<?= $end_time ?>" placeholder="结束时间"><em class="add-on2"><i style="font-size: 22px;position: relative;left: 2px;" class="iconfont icon-rili"></i></em><span style="line-height: 25px;margin-left: 8px;"><?= _('订单编号') ?></span><input type="text" name="return_code" class="A" style=" margin-left: 2px;width: 150px;" value="<?= $order_id ?>" placeholder="订单编号"> <a href="javascript:void(0);" class="sous" style="margin-right: 0;"><i class="iconfont icon-btnsearch"></i><?= _('搜索') ?></a><div style="clear:both;"></div></p>
                </form>
            </div>
        </div>
        ...
        <table style="width: 100%;" class="icos">
            ...
            <tr class="order_tit">
                <?php if ($data['state'] == 2)
                {
                    echo '<th colspan="2" width="411">' . _('商品') . '</th>';
                } ?>

                <th width="<?php if ($data['state'] == 2){echo 221;}else{echo 455;} ?>"><?= _('退款金额') ?></th>
                <th width="<?php if ($data['state'] == 2){echo 311;}else{echo 431;} ?>"><?= _('审核状态') ?></th>
                <th width="<?php if ($data['state'] == 2){echo 89;}else{echo 146;} ?>"><?= _('操作') ?></th>
            </tr>
            </tbody>
            <tbody>
            <tr>
                    ...
                    <td width="345" style="text-align: left;">' . $value['order_goods_name'] . '</td>';
                        ...
                        <td><span><a
                                    href="./index.php?ctl=Buyer_Service_Return&met=index&act=detail&id=<?= $value['order_return_id'] ?>"><i
                                        class="iconfont icon-chakan"></i>查看</a></span></td>
                    </tr>
                    </tbody>
                <?php } ?>
