...
input.error[type="text"], input.error[type="password"], textarea.error {
    ...
    outline: 0 none;
      ...
      <div class="member_infor_content">
        <div class="tabmenu">
          <ul class="tab pngFix">
           <li class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=friend"><?=_('查找好友')?></a></li>
			<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=friend&op=follow"><?=_('我关注的')?></a></li>
            <li><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=friend&op=fan"><?=_('关注我的')?></a></li>
			</ul>
        </div>
	<div class="ncm-friend-find"> 
          ...
          <form id="search_form" method="post" action="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=friend">
            <div class="search-form">
              <div class="normal">
                <?=_('会员名：')?><input type="text" class="text w300" name="searchname" id="searchname" value="<?=$user_name?>">
                <a class ="bbc_btns ncbtn ncbtn-mint" ><?=_('会员搜索')?></a></div>
            </div>
          </form>
			...
			<ul class="ncm-friend-list">
				...
				<li id="recordone_76">
				  ...
				  <dl class="info">
					...
					<dd></dd>
					<dd><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Message&met=message&op=sendMessage&id=<?=$v['user_id']?>" title="<?=_('站内信')?>"><i class="icon-envelope"></i><?=_('站内信')?></a> </dd>
				  </dl>
				  <div class="follow">
					
					<a href="javascript:void(0)" class="ncbtn-mini-blank bbc_btns" <?php if($v['status']==0){?>style="display:none;"<?php }else{?>style="display:block;"<?php }?>><?=_('已关注')?></a>
					<a class="ncbtn-mini ncbtn-mint bbc_btns"  data-param="{'ctl':'Buyer_User','met':'addFriendDetail','id':'<?=$v['user_id']?>'}" <?php if($v['status']==1){?>style="display:none;"<?php }else{?>style="display:block;"<?php }?> href="javascript:void(0)"><?=_('加关注')?></a>
...
$(".ncbtn-mint").click(function(){
	...
	$.post(SITE_URL  + '?ctl='+data_str.ctl+'&met='+data_str.met+'&typ=json',{id:data_str.id},function(data){
		if(data && 200 == data.status){
			Public.tips.success("<?=_('关注成功！')?>");
			e.prev().css('display','block');
			e.css('display','none');
			
		}else
		{
			Public.tips.error("<?=_('关注失败！')?>");
		}
	});
            ...
            <dl>
              ...
              <dd>
                ...
                <div class="content" nctype="content<?=$val['user_tag_id']?>">
                  ...
                  <div class="friends">
                    <h5><?=_('这里有')?><strong><?=$val['count']?></strong><?=_('个同样兴趣的用户：')?></h5>
					<?php if(!empty($val['user']['items'])){?>
                     <div>
                      ...
                      <a href="javascript:void(0);" class="care bbc_btns" nctype="batchFollow" data-param="{'ctl':'Buyer_User','met':'addFriends','id':'<?=$val['user_tag_id']?>'}"><?=_('+关注TA们')?></a> </div>
					<?php }else{?>
					   <?=_('暂时没有发现贴上该标签的用户。')?>
						<a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=tag"><?=_('点击这里贴上自己的兴趣爱好。')?></a>
					<?php }?>
                    </div>
                </div>
              </dd>
