...
<div class="aright">
      <div class="member_infor_content">
        <div class="tabmenu">
          <ul class="tab">
			<li class="active"><a><?php if($act=='add'){?><?=_('添加新')?><?php }else{?><?=_('编辑')?><?php }?><?=_('地址')?></a></li>
			</ul>
</div>
<div class="form-style-layout">
    ...
    <form id="form" name="form" action="" method="post">
        ...
        <dl>
            ...
            <dd><input type="hidden" name="address_area" id="t" value="<?=@$data['user_address_area']?>" />
					...
					<?php if(@$data['user_address_area']){ ?>
						<div id="d_1"><?=@$data['user_address_area'] ?>&nbsp;&nbsp;<a href="javascript:sd();"><?=_('编辑')?></a></div>
					<?php } ?>
		...
		<dl>
            <dt><?=_('设为默认地址：')?></dt>
            <dd><input type="checkbox" name="user_address_default" id="user_address_default" class="checkbox" value="1" <?php if(!empty($data)){?> <?php if($data['user_address_default'] == '1'){?>checked <?php }?><?php }?>>&nbsp;<?=_('设置为默认收货地址')?></dd>
        </dl>
        <dl class="foot">
            <dt>&nbsp;</dt>
            <dd><input class="save bbc_btns" type="submit" value="<?=_('提交')?>" class="submit"></dd>
        </dl>
    </form>
    </div>
...
<div class="form-style-layout">
    ...
    <form id="form" name="form" action="" method="post">
        ...
        <dl class="foot">
            <dt>&nbsp;</dt>
            <dd><input class="save" type="submit" value="<?=_('提交')?>" class="submit"></dd>
        </dl>
    </form>
    </div>
<script type="text/javascript" src="<?=$this->view->js?>/district.js"></script>
...
<div class="aright">
      <div class="member_infor_content">
        <div class="tabmenu">
          <ul class="tab">
			<li class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=address"><?=_('收货地址')?></a></li>
			</ul>
</div>
<div class="order_content_title clearfix">
<div class="clearfix" style="margin-top: 10px;">
<div class="ptkf bbc_btns">
 <!-- <a class="bbc_bg" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=address&act=edit_delivery" class="ncbtn ncbtn-bittersweet" style="right: 100px;" nc_type="dialog" dialog_title="<?=_('使用代收货（自提）')?>" dialog_id="daisou" uri="index.php?act=member_address&amp;op=delivery_add" dialog_width="900" title="<?=_('使用自提服务站')?>"><?=_('使用自提服务站')?></a>-->
<a class="" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_User&met=address&act=add" nc_type="dialog" dialog_title="<?=_('新增地址')?>" dialog_id="my_address_edit" uri="index.php?act=member_address&amp;op=address&amp;type=add" dialog_width="550" title="<?=_('新增地址')?>"><?=_('新增地址')?></a>
</div>
</div>
</div>
<table class="ncm-default-table annoc_con">
    <thead>
    <tr class="bortop">
        <th class="w80"><?=_('收货人')?></th>
        <th class="w200"><?=_('所在地区')?></th>
        <th class="w200"><?=_('街道地址')?></th>
        <th class="w120"><?=_('电话/手机')?></th>
        <th class="w100"></th>
