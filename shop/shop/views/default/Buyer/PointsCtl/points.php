        ...
        <div class="order_content">
          <div class="div_head tabmenu clearfix">
            <ul class="tab clearfix">
              <li  class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Points&met=points"><?=_('积分明细')?></a></li>
              <li> <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Points&met=points&op=getPointsOrder"><?=_('兑换记录')?></a></li>
            </ul>
          </div>
          <ul>
            <li>
              <div class="operation">
                <p class="p_title"><?=_('积分获得规则：')?></p>
                <div style="margin-top:10px; margin-left:10px;">
                  <p class="p_content"><?=_('成功注册会员；增加')?><span style="color:#F06"><?=$web['points_reg'] ?></span><?=_('积分；
                    会员每天登陆；增加')?><span style="color:#F06"><?=$web['points_login'] ?></span><?=_('积分；评价完成订单；增加')?><span style="color:#F06"><?=$web['points_evaluate'] ?></span> <?=_('积分')?></p>
                  <p class="p_content"><?=_('购物并付款成功后将获得订单总价÷')?><?=$web['points_recharge'] ?><?=_('（最高限额不超过')?><?=$web['points_order'] ?><?=('）')?><?=_('积分')?></p>
                  <p class="p_content"><?=_('如订单发生退款、退货等问题时，积分将不给予退换')?></p>
                </div>
              </div>
              <div>
			  ...
			  <form action="index.php" id="search_form" method="get">
				...
				<p class="order_types">
					<?=_('积分总数：')?> <span style="color:#F00"><?=$this->user['points']['user_points'];?></span>
				</p>
