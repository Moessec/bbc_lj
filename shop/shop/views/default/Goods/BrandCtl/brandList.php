...
<div class="goods_det_about clearfix">
    <div class="wrap clearfix">
        ...
        <p style="width:183px;">品牌<i class="iconfont icon-iconjiantouyou"></i> <span style="color:#000"><?=$data_brand['brand_name'] ?></span></p>
        </div>
...
<div class="wrapsa clearfix">
    <div class="wrapleftS clearfix">
        <div class="divleft">
            ...
            <p style="color:#CCC; margin:10px 0px;"><?=$data_brand['brand_collect']; ?>人已关注</p>
            <a class="btn bbc_bg">
                <?php if(!isset($data_favorites)||empty($data_favorites)){ ?>
                    <div  onclick="collectBrand(<?=($data_brand['brand_id'])?>)" class="brand_list_d"><i class="iconfont icon-iconxihuan"></i>关注</div>
                <?php }else{ ?>
                    <div  onclick="canleCollectBrand(<?=($data_brand['brand_id'])?>)" class="cancel_save"><i class="iconfont icon-iconxihuan"></i>取消关注</div>
                <?php } ?>
            </a>
        </div>
        <div class="divleft_2">
            <p>品牌信息 <span class="Fcolor">Information</span></p>
            <p style="color:#CCC;font-weight:bold">--</p>
            <p><span  class="Fcolor">品牌名:</span><?=$data_brand['brand_name'] ?></p>
    ...
    <div class="wrapright">
        <div class="martop">
            <div class="divhead clearfix">
                <p style="float:left" class="fontc">品牌商品  <b style="color:#CCC; font-weight:100">new</b> </p>
                <p style="float:right"><a class="aHover" href="index.php?ctl=Goods_Goods&met=goodslist&brand_id=<?=$data_brand['brand_id']; ?>">查看全部新品 <i class="iconfont icon-youyuan"></i></a></p>
            </div>
            <div class="paipaisy_div_1"> <a class="itemList-prev  iconfont icon-iconjiantoushang" ></a>
                <ul class="ml">
                    <?php if(!empty($data_goods_common)):
                        ...
                        ?>
                                <li><a href="index.php?ctl=Goods_Goods&met=goods&gid=<?=$value['goods_id'] ?>"><img src="<?=$value['common_image'] ?>" width="190px" /></a>
                                    <p class="iLsc-date bbc_bg"><?=$value['common_add_time'] ?></p>
                                    <p class="Price bbc_color">￥<?=$value['common_price'] ?></p>
                                    <p class="pFont"><a href="index.php?ctl=Goods_Goods&met=goods&gid=<?=$value['goods_id'] ?>"><?=$value['common_name'] ?></a></p>
                                </li>
                    <?php
                            endforeach;
                        endif; ?>
        ...
        <div  class="martop">
            <div class="divhead clearfix">
                <p style="float:left" class="fontc">大家都在买<b style="color:#CCC; font-weight:100">  hot 	</b> </p>
                <p style="float:right"><a class="aHover" href="index.php?ctl=Goods_Goods&met=goodslist&brand_id=<?=$data_brand['brand_id'];?>">查看更多<i class="iconfont icon-youyuan"></i></a></p>
            </div>
            <ul  class="Coupons clearfix">
                <?php if(!empty($data_all_buy)): foreach($data_all_buy as $k=>$v): ?>
                    <li>
                        <a href="index.php?ctl=Goods_Goods&met=goods&gid=<?=$v['goods_id'] ?>"><img src="<?=$v['common_image'] ?>" width="200"  /></a>
                        <div>
                            <p class="flagShipRed bbc_color" style="display:inline-block;">￥<?=$v['common_price'] ?> </p>
                            <p style="display:inline-block; margin-left:15px;" class="del">￥<?=$v['common_market_price'] ?></p>
                            <br />
                            <p class="pFont"><a href="index.php?ctl=Goods_Goods&met=goods&gid=<?=$v['goods_id'] ?>"> <?=$v['common_name'] ?></a></p>
                            <p class="font" style="display:inline-block;">销量:<span  style="color:#005aa0;margin-left:4px;"><?=$v['common_salenum'] ?></span></p>
