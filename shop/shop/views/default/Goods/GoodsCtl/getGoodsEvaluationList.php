...
<li style="display:block">
    <div class=" comment_1 clearfix">
        <div class="sppl">
            <div class="HPQ">
                <p class="font_1 bbc_color"><?=($good_pre)?><?=_('%')?></p>

                <p class="font_2"><?=_('好评价')?></p>
            </div>
            <div class="percent">
                <dl>
                    <dt><?=_('好评')?><span>(<?=($good_pre)?>%)</span>
                    </dt>
                    <dd>
                        <div>
                ...
                <dl>
                    <dt><?=_('中评')?><span>(<?=($middle_pre)?><?=_('%')?>)</span>
                    </dt>
                    <dd class="d1">
                        <div>
                ...
                <dl>
                    <dt><?=_('差评')?><span>(<?=($bad_pre)?><?=_('%')?>)</span></dt>
                    <dd class="d1">
                        <div>
        ...
        <ul class="goods_det_about clearfix evaluation">
            <li><a name="all" <?php if($type == 'all'): ?> class="checked" <?php endif;?>><?=_('全部评论')?>(<?=($all_count)?>)</a></li>
            <li><a name="image" <?php if($type == 'image'): ?>class="checked" <?php endif;?>> <?=_('晒图')?>(<?=($img_count)?>)</a></li>
            <li><a name="good" <?php if($type == 'good'): ?>class="checked" <?php endif;?>><?=_('好评')?>(<?=($good_count)?>)</a></li>
            <li><a name="middle" <?php if($type == 'middle'): ?>class="checked" <?php endif;?>> <?=_('中评')?>(<?=($middle_count)?>)</a></li>
            <li><a name="bad" <?php if($type == 'bad'): ?>class="checked" <?php endif;?>> <?=_('差评')?>(<?=($bad_count)?>)</a></li>
        </ul>
        <?php
        if(!empty($data['items'])):
            ?>
                ...
                <?php foreach($data['items'] as $evkey=>$eval): ?>
                    ...
                    <div class="comment clearfix">
                        ...
                        <div class="detaildiv_2 ">
                            ...
                            <?php if($evval['explain_content']): ?><p><?=_('[解释]')?><?=($evval['explain_content'])?></p><?php endif;?>
                            <?php if(isset($eval[1])): ?><br/><p><?=_('[追加]')?><?php Text_Filter::filterWords($eval[1]['content']);?><?=($eval[1]['content'])?>
                            ...
                            <?php if($eval[1]['explain_content']): ?><p><?=_('[解释]')?><?php Text_Filter::filterWords($eval[1]['explain_content']);?><?=($eval[1]['explain_content'])?></p><?php endif;?>
                            <?php endif; ?>
