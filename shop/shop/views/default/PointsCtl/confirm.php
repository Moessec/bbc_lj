	...
	<div class="wrap">
		...
		?>
			<li class="<?php if($value['user_address_default'] == 1){?>add_choose<?php }?>" id="addr<?=($value['user_address_id'])?>">
				<div class="editbox">
					<a onclick="edit_address(<?=($value['user_address_id'])?>)"><?=_('编辑')?></a>
					<a onclick="del_address(<?=($value['user_address_id'])?>)"><?=_('删除')?></a>
				</div>
		...
		<div class="cart_goods">
			<ul class='cart_goods_head cart_goods_head_return clearfix'>
				...
				<li style="width:140px;"><?=_('兑换积分')?></li>
				<li style="width:140px;"><?=_('兑换数量')?></li>
				<li style="width:140px;"><?=_('积分小计')?></li>
			</ul>
			...
			<div class="frank clearfix">
				...
				<p class="submit submit2"><span><?=_('所需总积分')?>：<i class="total bbc_color"><?=(number_format($data['total_points']) )?></i> <?=_('积分')?></span><a id="pay_btn" class="bbc_bg"><?=_('确认兑换')?></a></p>
			</div>
		</div>
	</div>
