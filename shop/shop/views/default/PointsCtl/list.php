...
<div class="bbc-breadcrumb-layout">
	<div class="bbc-breadcrumb wrapper"><i class="icon-home"></i> <span><a href="<?=Yf_Registry::get('url')?>">首页</a></span><span class="arrow"><i class="iconfont icon-iconjiantouyou"></i></span> <span><a href="<?=Yf_Registry::get('url')?>?ctl=Points&met=index">积分中心</a></span><span class="arrow"><i class="iconfont icon-iconjiantouyou"></i></span> <span><a href="<?=Yf_Registry::get('url')?>?ctl=Points&met=pList">兑换礼品列表</a></span> </div>
</div>
...
<div class="bbc-container">
	...
	<div class="bbc-member-top">
		...
		<div class="bbc-memeber-pointcart"> <a href="<?=Yf_Registry::get('url')?>?ctl=Points&met=pointsCart" class="btn bbc_bg_col"><?=_('礼品兑换购物车')?><em><?=$data['points_cart_num']?></em></a></div>
	</div>
	<?php } ?>
	<div class="bbc-main-layout">
		<div class="bbc-category">
			<dl class="searchbox">
				...
				<dd>
				<ul>
                    <li onclick='list("default")' <?php if(!request_string('orderby') || request_string('orderby') =='default'){ ?>class="hova" <?php }else{ ?> op_type="search_orderby" data-param='{"orderval":"default"}' style="cursor: pointer;" <?php } ?>><?=_('默认排序')?></li>
                    <li op_type="search_orderby" class="<?=(request_string('orderby')=='pointsdesc' || request_string('orderby')=='pointsasc')?'hova':''?>" onclick='list(<?=request_string('orderby')=='pointsdesc'?'"pointsasc"':'"pointsdesc"'?>)'><?=_('积分值')?>
                    ...
                    <li op_type="search_orderby" class="<?=(request_string('orderby')=='stimedesc' || request_string('orderby')=='stimeasc')?'hova':''?>" onclick='list(<?=request_string('orderby')=='stimedesc'?'"stimeasc"':'"stimedesc"'?>)'><?=_('上架时间')?>
                        <em class="display_arrow">
					...
					<li><?=_('会员等级')?>：
						<select id="level" onchange="level(this);">
							<option value="0" selected=""><?=_('-请选择-')?></option>
					...
					<li><?=_('所需积分')?>：
						...
						<a href="javascript:searchPointsGoods();" class="bbcbtn bbc_btns"><?=_('搜索')?></a> </li>
					<li>&nbsp;</li>
					<li>
