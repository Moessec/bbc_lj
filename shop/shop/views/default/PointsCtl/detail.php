...
<div class="bbc-breadcrumb-layout">
    <div class="bbc-breadcrumb wrapper"><i class="icon-home"></i>
        <span><a href="<?=Yf_Registry::get('url')?>"><?=_('首页')?></a></span><span class="arrow"><i class="iconfont icon-iconjiantouyou"></i></span>
        <span><a href="<?=Yf_Registry::get('url')?>?ctl=Points&met=index"><?=_('积分中心')?></a></span><span class="arrow"><i class="iconfont icon-iconjiantouyou"></i></span>
        <span><?=_('兑换礼品详情')?></span>
    </div>
</div>
	...
	<div class="bbc-detail">
		...
		<div class="bbc-gift-summary">
			...
			<div class="bbc-meta">
				...
				<dl>
					<dt><?=_('所需积分')?>：</dt>
					<dd class="points"> <strong><em class="bbc_color"><?=$data['goods_detail']['points_goods_points']?></em> <?=_('积分')?></strong><span class="bbc_btns"><?=$data['goods_detail']['points_goods_limitgrade_label']?><?=_('专享')?></span></dd>
				</dl>
            </div>
			<div class="bbc-key">
				<dl>
					...
					<dd class="bbc-figure-input">
						...
						<span>（<?=_('剩余数量')?>：<?=$data['goods_detail']['points_goods_storage']?><input id="storagenum" value="<?=$data['goods_detail']['points_goods_storage']?>" type="hidden">）</span>
					</dd>
				</dl>
				<input id="limitnum" value="<?=$data['goods_detail']['points_goods_limitnum']?>" type="hidden">
			</div>
			<div class="ncs-btn">
				<?php if($data['goods_detail']['sell_state'] == Points_GoodsModel::WILLSTART){ ?><a class="no-buynow"><?=_('即将开始')?></a>
				<?php }elseif($data['goods_detail']['sell_state'] == Points_GoodsModel::ENDEXCHANGE){ ?><a class="no-buynow"><?=_('兑换结束')?></a>
				<?php }elseif($data['goods_detail']['sell_state'] == Points_GoodsModel::ONEXCHANGE){ ?>
                <a class="buynow bbc_btns" onclick="return add_to_cart();" style="cursor:pointer;"><i class="ico"></i><?=_('我要兑换')?></a>
				<?php }?>
            </div>
		</div>
		<div style=" position: absolute; z-index: 1; top: -1px; right: -1px;">
			<div class="bbc-info">
				<div class="title"><h4><?=_('商城提供')?></h4></div>
				<div class="content">
					<dl>
						<dt><?=_('礼品编号')?>：</dt>
				...
				<div class="title"><h4><?=_('兑换记录')?></h4></div>
					...
					<ul class="recommend">
						...
						<li>
							...
							<div class="pgoods-points">
								<span class="bbc_color"><?=$value['points_goods_points']?></span><?=_('积分')?>
								<?php if($value['points_goods_limitgrade']) 
								{
								?>
									<span class="bbc_btns"><?=$value['user_grade_limit_label']?><?=_('专享')?></span>
								<?php 
								}
								?>
							</div>
		...
		<div class="bbc-goods-main">
			<div class="tabbar">
				<div class="bbc-goods-title-nav">
					<ul id="categorymenu">
						<li class="current"> <a id="tabGoodsIntro" href="#intro"><?=_('礼品介绍')?></a> </li>
					</ul>
				</div>
			</div>
