...
<div class="wrap wrap_w">
	...
	<?php if(!$data){?>
		<div class="cart_empty clearfix">
			...
			<div class="empty-warn">
				<p><?=_('您还没有选择兑换礼品')?></p>
				<div>
					<a href="<?=Yf_Registry::get('url')?>?ctl=Points&met=pList"><span class="iconfont icon-mashangqugouwu"></span><?=_('马上去兑换')?></a>
					<a href="<?=Yf_Registry::get('url')?>?ctl=Buyer_Points&met=points&op=getPointsOrder"><span class="iconfont icon-chakandingdan"></span><?=_('查看已兑换信息')?></a>
				</div>
			</div>
		</div>
	<?php }else{?>
	...
	<div class="cart_goods">
		<ul class='cart_goods_head clearfix'>
            ...
            <li class="price_all"><?=_('小计(积分)')?></li>
             <li class="goods_num"><?=_('数量')?></li>
            <li class="goods_price"><?=_('单价(积分)')?></li>
	...
	<div class="pay_fix">
		<div class="wrap wrap3 cart-checkbox">
			<div class="clearfix cart_pad">
				...
				<div class="cart-sum">
					<span><?=_('所需总积分')?>：</span>
					<strong class="Price"><em class="subtotal subtotal_all bbc_color"><?=$data['total_points']?></em><?=_('积分')?></strong>
				</div>
			</div>
		</div>
	</div>
