...
<div class="toolbar-wrap J-wrap">
				<div class="toolbar">
					<div class="toolbar-panels J-panel">
						<div style="visibility: hidden;" class="J-content toolbar-panel tbar-panel-news toolbar-animate-out ">
							<div class="toolbar-panelff">
								<div class="padd2">
									...
									<p class="view_all"><a href="http://shop.bbc-builder.com/index.php?ctl=Buyer_Cart&amp;met=cart">全屏查看</a></p>
								</div>
						...
						<div style="visibility: hidden;" class="J-content toolbar-panel tbar-panel-cart toolbar-animate-out">
							<div class="padd2">
								...
								<p class="view_all"><a href="<?=Yf_Registry::get('url')?>?ctl=Buyer_Cart&met=cart"><?=_('全屏查看')?></a></p>
							</div>
							<?php $count = 0;
								...
								<div class="cart_contents">
									<div class="cart_contents_head">
										...
										<div class="cart_contents_title">
											<a href="<?= Yf_Registry::get('url') ?>?ctl=Shop&met=Index&id=<?=($cartv['shop_id'])?>"><span title="<?=($cartv['shop_name'])?>"><?=($cartv['shop_name'])?></span></a><?php if(isset($cartv['mansong_info']['rule_discount'])){?>
												<?=_('（促销）')?>
											<?php }?>
										</div>
										...
										<div class="cart_list">
											<div class="cart_list_order clearfix">
												...
												<div class="cart_list_orderimg">
													<a href="<?=Yf_Registry::get('url')?>?ctl=Goods_Goods&met=goods&gid=<?=($cartgv['goods_id'])?>" target="_blank">
						...
						<div style="visibility: hidden;" class="J-content toolbar-panel tbar-panel-assets toolbar-animate-out">
							<div class="padd">
								<p ><a href="#" class="close_p"><?=_('关闭')?><i class="iconfont icon-youshaungjiantou"></i></a><a href="<?= Yf_Registry::get('paycenter_api_url') ?>" class="view_all"><?=_('全屏查看')?></a></p>
						...
						<div style="visibility: hidden;" class="J-content toolbar-panel tbar-panel-follow toolbar-animate-out">
							<div class="padd">
								<p ><a href="#" class="close_p"><?=_('关闭')?><i class="iconfont icon-youshaungjiantou"></i></a><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Favorites&met=favoritesShop" class="view_all"><?=_('全屏查看')?></a></p>
								<div class="item_cons">
									...
									<div class="item">
										...
										<a class="barnd_shop" href="<?= Yf_Registry::get('url') ?>?ctl=Shop&met=index&typ=e&id=<?=$v['shop_id']?>"><?=_('进入店铺')?></a>
										<div class="brand_goodsList">
											<?php if(!empty($v['detail']['items'])){?>
											<?php foreach($v['detail']['items'] as $kk=>$vv){ ?>
											<a href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?=$vv['goods_id']?>">
						...
						<div style="visibility: hidden;" class="J-content toolbar-panel tbar-panel-history toolbar-animate-out">
							<div class="padd over">
                                                            <p ><a href="#" class="close_p"><?=_('关闭')?><i class="iconfont icon-youshaungjiantou"></i></a><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Favorites&met=footprint" class="view_all"><?=_('全屏查看')?></a></p>
									<ul class="history_goods clearfix">
										<?php if(!empty($footprint_list['items'])){?>
										<?php
											foreach($footprint_list['items'] as $k=>$v){ ?>
										<?php if(!empty($v['detail'])){?>
										<li><a href="<?=Yf_Registry::get('url')?>?ctl=Goods_Goods&met=goods&gid=<?=$v['detail']['goods_id']?>"><img src="<?php if(!empty($v['detail']['common_image'])){?><?=image_thumb($v['detail']['common_image'],116,116)?><?php }else{?><?= image_thumb($this->web['goods_image'],116,116)?><?php }?>"/><h5><?=$v['detail']['common_name']?></h5><h6  class="bbc_color"><?=format_money($v['detail']['common_price'])?></h6></a></li>
										<?php }?>
						...
						<div style="visibility: hidden;" class="J-content toolbar-panel tbar-panel-sav toolbar-animate-out">
							<div class="padd over">
								<p class="padd2"><a href="#" class="close_p"><?=_('关闭')?><i class="iconfont icon-youshaungjiantou"></i></a><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Favorites&met=favoritesGoods" class="view_all"><?=_('全屏查看')?></a></p>
									<ul class="sav_goods clearfix">
										<?php if(!empty($goods_list['items'])){?>
										<?php
										foreach($goods_list['items'] as $k=>$v){ ?>
										<?php if(!empty($v['detail'])){?>
										<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&gid=<?=$v['goods_id']?>"><img src="<?php if(!empty($v['detail']['goods_image'])){?><?=image_thumb($v['detail']['goods_image'],116,116)?><?php }else{?><?= image_thumb($this->web['goods_image'],116,116)?><?php }?>"/><h5><?=$v['detail']['goods_name']?></h5><h6  class="bbc_color"><?=format_money($v['detail']['goods_price'])?></h6></a></li>
										<?php }?>
										<?php }?>
										<?php }else{?>
					...
					<div class="toolbar-tabs J-tab">
						<div class="nav_head"><img src="<?php if(empty($this->user_info['user_logo'])){  ?><?= Yf_Registry::get('ucenter_api_url') ?>?ctl=Index&met=img&user_id=<?= @Perm::$userId ?> <?php }else{ ?><?php echo $this->user_info['user_logo'];}?>"/></div>
...
<div class="toolbar-footer">
	<div class="code_screen">
		...
		<p class="code_cont" >
			<img src="<?= $this->view->img ?>/code.png"/>
		</p>
	</div>
