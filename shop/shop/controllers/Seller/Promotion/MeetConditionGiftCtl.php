...
class Seller_Promotion_MeetConditionGiftCtl extends Seller_Controller
{
				...
				if ($flag)
				{
					foreach ($mansong_rule as $key => $rule)
					{
						...
						if($field_rule_row['goods_id'])
						{
							...
							if ($goods_row['shop_id'] == Perm::$shopId && is_numeric($field_rule_row['rule_price']) && $field_rule_row['rule_price'] > 0 && is_numeric($field_rule_row['rule_discount']) && $field_rule_row['rule_discount'] > 0)
							{
	...
	public function addCombo()
	{
		...
		if($month > 0)
		{
			...
			$field_row['cost_desc']   = _('店铺购买满送活动消费');
			...
			if (is_ok($rs_row) && $this->manSongQuotaModel->sql->commitDb())
			{
				$msg    = _('操作成功');
				$status = 200;
			}
