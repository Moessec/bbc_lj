...
class Seller_Promotion_IncreaseCtl extends Seller_Controller
{
	...
	public function combo()
	{
		...
		if('json' == $this->typ)
		{
			$data['promotion_increase_price'] = Web_ConfigModel::value('promotion_increase_price');
			$this->data->addBody(-140, $data);
		}
		else
		{
	...
	public function addCombo()
	{
		...
		$data        = array();
		$combo_row   = array();
		$month_price = Web_ConfigModel::value('promotion_increase_price');
        ...
        if($month > 0)
        {
            ...
            if (is_ok($rs_row) && $this->increaseComboModel->sql->commitDb())
            {
                $msg    = _('操作成功！');
                $status = 200;
            }
