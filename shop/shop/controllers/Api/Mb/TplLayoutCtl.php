...
class Api_Mb_TplLayoutCtl extends Api_Controller
{
    ...
    {
        $layout_list = $this->mbTplLayoutModel->getByWhere( array(), array('mb_tpl_layout_order' => 'ASC') );
    ...
    public function addTplLayout()
    {
        ...
        if ( !empty($item_type) )
        {
            ...
            {
                $check_data = $this->mbTplLayoutModel->getByWhere( array('mb_tpl_layout_type' => 'adv_list') );

                if ( !empty($check_data) )
                {
                    return $this->data->addBody(-140, array(), _('广告条板块只能添加一个'), 250);
                }
            }
    ...
    public function editSortTplLayout()
    {
        ...
        $this->data->addBody(-140, array(), _('success'), 200);
    }
