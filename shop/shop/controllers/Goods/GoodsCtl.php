...
// var_dump($cond_row);die;
			if($act&&$actorder)
			{
				...
				{
					// $cond_row['shop_id:>='] = $shop_id;
				$data = $Shop_catgoodslistModel->getCommonList($cond_row, $order_row = array(), $page = 1, $rows = 100);

				}else{

				$data = $Shop_catgoodslistModel->getCommonList($cond_row, $order_row = array("$act"=>"$actorder"), $page = 1, $rows = 100);
				}
	...
	public function index222()
	{
		...
		$QuotaModel = new Discount_QuotaModel();
        ...
        else
        {
            ...
            $data               = $discountBaseModel->getDiscountActList($cond_row, array('discount_id' => 'DESC'), $page, $rows);
            $Yf_Page->totalRows = $data['totalsize'];
            $page_nav           = $Yf_Page->prompt();
        }
...
function cur_goodslist()
{
       $shop_id = request_int('shop_id');
		...
		$data = $curgoodsModel->getCommonList(array('shop_id'=>$shop_id),array('common_edit_time'=>'DESC'));
		// var_dump($data);die;
	if ('json' == $this->typ)
		{
			$this->data->addBody(-140, $data);
