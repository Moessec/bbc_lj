$(document).ready(function() {
    ...
    decoration.show_dialog_module_html = function($dialog, content) {
        $dialog.nc_show_dialog({width: 1016, title: '自定义模块'});
        if(!decoration.editor) {
           decoration.editor = UE.getEditor('module_html_editor', {
            toolbars: [
                [
                 'source','bold', 'italic', 'underline', 'forecolor', 'backcolor', 'justifyleft', 'justifycenter', 'justifyright', 'insertunorderedlist', 'insertorderedlist', 'blockquote',
         ...
         decoration.editor.ready(function() {
           decoration.editor.setContent(content);
        });
//        decoration.editor.html(content);
    ...
    decoration.show_dialog_module_goods = function($dialog, content) {
        ...
        $(content).find('[nctype="goods_item"]').each(function() {
            $(this).append('<a class="lblock bbc_img_btns" nctype="btn_module_goods_operate" href="javascript:;"><i class="iconfont icon-quxiao" style="font-size: 16px;"></i>取消选择</a>');
...
//    });
    ...
    $('#btn_save_module_html').on('click', function() {
     ...
     //   var html = $('#module_html_editor').val();
        var html = decoration.editor.getContent();
    ...
    $('#btn_add_slide_image').on('click', function() {
        ...
        if(image_count >= decoration.slide_image_limit) {
            alert('每个幻灯片最多只能上传' + decoration.slide_image_limit + '张图片');
            return;
        }
...
//    });
    ...
    $('#btn_save_add_slide_image').on('click', function() {
        var data = {};
        $image = $('#img_slide_image');
        if($image.attr('src')) {
            data.image_url = $image.attr('src');
            data.image_name = $('#input_slide_image').val();
...
//    });
    ...
    $('#div_module_goods_search_list').on('click', '[nctype="btn_module_goods_operate"]', function() {
        ...
        $goods.find('[nctype="btn_module_goods_operate"]').html('<i class="iconfont icon-quxiao" style="font-size: 16px;"></i>取消选择');
    ...
    $('#btn_save_module_goods').on('click', function() {
        ...
        $('#div_module_goods_list').find('[nctype="goods_item"]').each(function() { 
            ...
            goods.common_name = $(this).attr('data-goods-name');
            goods.common_price = $(this).attr('data-goods-price');
            goods.common_image = $(this).attr('data-goods-image');
            data[i] = goods;
            i++;
        });
