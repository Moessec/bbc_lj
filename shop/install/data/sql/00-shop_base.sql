...
CREATE TABLE `yf_base_district` (
  ...
  KEY `area_parent_id` (`district_parent_id`)
...
CREATE TABLE `yf_invoice` (
  ...
  `invoice_state` enum('1','2','3') DEFAULT NULL COMMENT '1普通发票2电子发票3增值税发票',
...
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='消费者保障服务类型表';
...
CREATE TABLE `yf_transport_item` (
  ...
  KEY `temp_id` (`transport_type_id`,`logistics_type`)
