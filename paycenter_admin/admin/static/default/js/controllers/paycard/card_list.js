...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name:'card_code', label:'卡片激活码',  width:100, align:"left",align:"center"},
            ...
            {name:'card_money', label:'充值卡余额', width:100, align:'center'},
            {name:'card_froze_money', label:'卡冻结金额', width:100, align:'center'}
...
var handle = {
    operate: function (t, e)
    {
        if ("add" == t)
        {
            var i = "新增充值卡", a = {oper: t, callback: this.callback, card_row:card_row};
        }
        else
        {
            var i = "修改充值卡", a = {oper: t, rowData: $("#grid").jqGrid('getRowData',e), callback: this.callback};
        }
        $.dialog({
            title: i,
    ...
    }, del: function (t)
    {
        $.dialog.confirm("删除的充值卡将不能恢复，请确认是否删除？", function ()
        {
...

            {
                if (e && 200 == e.status)
                {
                    parent.Public.tips({content: "充值卡删除成功！"});
                    $("#grid").jqGrid("delRowData", t)
                }
                else
                {
                    parent.Public.tips({type: 1, content: "充值卡删除失败！" + e.msg})
                }
            })
        })
    }
