...
class IndexCtl extends Yf_AppController
{
	...
	public function db()
	{
		...
		$db_row = current($db_cfg['db_cfg_rows']['master']['shop_admin']);
		include $this->view->getView();
	}
	...
	public function install()
	{
		...
		else if (2 == $state_row['state'])
		{
			...
			foreach ($dir as $item)
			{
				...
				if (is_file($file))
				{
					$flag = $db->import($file, TABEL_PREFIX);
...
function check_install_db()
{
	...
	if ($db_row = current($db_cfg['db_cfg_rows']['master']['shop_admin']))
	{
		try
		{
			$db_id = 'shop_admin';
