...
if(urlParam.otyp==1 || urlParam.otyp==3){
	var cols = [
		{name:'operating', label:'操作', width:30, fixed:true, formatter:operFmatter, align:"center"},
		{name:'return_code', label:'退单编号', width:208, align:"center"},
		...
		{name:'return_add_time', label:'申请时间',  width:140, align:"center",sortable:true},
		{name:'return_shop_message', label:'商家处理备注',  width:300, align:"center"},
		{name:'return_shop_time', label:'商家处理时间',  width:140, align:"center",sortable:true},
		{name:'order_number', label:'订单编号',  width:208, align:"center"},
	...
	var cols = [
		...
		{name:'return_reason', label:'申请原因',  width:300, align:"center"},
		{name:'return_add_time', label:'申请时间',  width:140, align:"center",sortable:true},
		{name:'order_goods_name', label:'涉及商品',  width:300, align:"center"},
		{name:'return_shop_message', label:'商家处理备注',  width:300, align:"center"},
		{name:'return_shop_time', label:'商家处理时间',  width:140, align:"center",sortable:true},
		{name:'order_number', label:'订单编号',  width:208, align:"center"},
		{name:'buyer_user_account', label:'买家',  width:150, align:"center"},
...
function operFmatter (val, opt, row) {
	...
	var THISPAGE = {
		init: function(data){
			...
			this.mod_PageConfig = Public.mod_PageConfig.init('return-wait-list');//页面配置初始化
			this.initDom();
			this.loadGrid();            
			this.addEvent();
		},
		...
		loadGrid: function(){
			...
			$("#grid").jqGrid({
				url:SITE_URL +  '?ctl=Trade_Return&met=getReturnWaitList&typ=json',
		...
		addEvent: function()
		{
			...
			$("#btn-excel").click(function ()
			{
				...
				window.open(SHOP_URL + "?ctl=Api_Trade_Return&met=getReturnWaitExcel&debug=1"+query);
			});
			//刷新
