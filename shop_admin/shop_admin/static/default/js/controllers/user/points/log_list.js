...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name:'points_log_id', label:'积分id', width:50, align:"center"},
         
            {name:'user_id', label:'会员id', width:100,align:'center'},
            {name:'user_name', label:'会员名称', width:150,align:'center'},
            {name:'points_log_points', label:'积分', width:120, align:"center"},
            {name:'points_log_time', label:'操作时间', width:135, align:"center"},
            {name:'points_log_desc', label:'操作描述', width:100, align:"center"},
            {name:'admin_name', label:'管理员名称', width:100, align:"center"},
            
        ];
        this.mod_PageConfig.gridReg('grid', colModel);
        colModel = this.mod_PageConfig.conf.grids['grid'].colModel;
        $("#grid").jqGrid({
            url:SITE_URL +  '?ctl=User_Points&met=getPointsList&typ=json',
  ...
  add: function (t, e)
    {
        ...
        parent.tab.addTabItem({
            tabid: f,
            text: '增加减少积分',
            url: SITE_URL + '?ctl=User_Points&met=addPoints'
        })
    }
