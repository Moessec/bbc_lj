...
function initGrid()
{
    ...
    }, {name: "rights_group_name", index: "rights_group_name", width: 150}/*, {name: "rights_group_rights_ids", index: "rights_group_rights_ids", width: 650}*/];
    $("#grid").jqGrid({
...
function settingDataFormatter(val, opt, row) {
    return '<div class="operating" data-id="' + row.rights_group_id + '"><a class="ui-icon ui-icon-pencil" title="详细设置授权信息" href="./index.php?ctl=Rights_Group&met=manage&rights_group_name=' + row.rights_group_name + '&rights_group_id=' + row.rights_group_id + '"></a></div>';
};
...
var handle = {
    ...
    del: function (t)
    {
        ...
        {
            Public.ajaxPost("./index.php?ctl=Rights_Group&met=remove", {rights_group_id: t}, function (e)
            {
                if (e && 200 == e.status)
                {
                    parent.Public.tips({content: "职员删除成功！"});
                    $("#grid").jqGrid("delRowData", t)
                }
                else
                {
                    parent.Public.tips({type: 1, content: "职员删除失败！" + e.msg})
                }
            })
        })
