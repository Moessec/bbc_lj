...
<script>
            ...
            $.get(SITE_URL + '?ctl=Goods_Goods&met=getCatGoodsList&typ=json&goods_cat_id='+gc_id+'&goods_name='+$('#goods_name').val(),function(a){
                if(a.status==200)
                {
                    ...
                    $.each(a.data['items'],function(key,val){
                        str+= "<li style='float:left;width:12%; margin:0 10px; border:2px solid #999;text-align:center;padding:10px; '>";
                        ...
                        str+="<i></i>";
                        str+="<img width='72' src="+val['common_image']+" goods_name="+val['common_name']+" goods_id="+val['common_id']+" title="+val['common_name']+">";
                        str+="</span>";
                        str+="</div>";
                        str+="<div class='goods-name' style='height:40px;overflow:hidden;'>";
    ...
    function select_recommend_goods(goods_id) {
        ...
        text_append += '<a href="http://b2b2c.shopnctest.com/tesa/shop/index.php?act=goods&goods_id='+goods_id+'" target="_blank">';
