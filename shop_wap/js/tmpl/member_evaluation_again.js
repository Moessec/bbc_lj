$(function () {
    ...
    $.getJSON(ApiUrl + "/index.php?ctl=Buyer_Order&met=evaluation&act=again&typ=json", {k: e,u:u, order_id: a}, function (r) {
        if (r.status == 250) {
            ...
            for(var ts = 0; ts < order_goods_id_list.length; ts++) {
                ...
                for( var tsi = 0; tsi < 4; tsi++ ) {
                    if ( l["goods[" + order_goods_id + "][evaluate_image][" + tsi + "]"].length > 0 ) {
                        ...
                        if ( l["goods[" + order_goods_id + "][evaluate_image][" + (tsi+1) + "]"].length > 0 ) {
                            imgurl += ','
                        }
                    }
