...
$(function ()
{
    ...
    $("#header").on("click", ".search-btn", function ()
    {
        ...
        window.location.href = WapSiteUrl + "/tmpl/store-list.html" + '?keywords='+keyword;
    });
...
function get_list()
{
    ...
    {
        var url = ApiUrl + "/index.php?ctl=Shop_Index&met=index&typ=json";
    }
...
function collectShop(shop_id){
    ...
    if (k && u)
    {
        $.getJSON(ApiUrl  + '/index.php?ctl=Shop&met=addCollectShop&typ=json',{shop_id:shop_id},function(data)
        {
            $.sDialog({
                skin: "red",
                content: data.msg,
