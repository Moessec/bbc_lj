...
function toPay(a, e, p) {
    $.ajax({
        ...
        data: {k: key, u: getCookie('id'), pay_sn: a},
        dataType: "json",
        success: function (p) {
            checkLogin(p.login);
            if (p.datas.error) {
                $.sDialog({skin: "red", content: p.datas.error, okBtn: false, cancelBtn: false});
                return false
            }
            ...
            $("#toPay").click(function () {
                if (payment_code == "") {
                    $.sDialog({skin: "red", content: "请选择支付方式", okBtn: false, cancelBtn: false});
                    return false
                }
                if (s) {
                    if (password == "") {
                        $.sDialog({skin: "red", content: "请填写支付密码", okBtn: false, cancelBtn: false});
                        return false
                    }
                    $.ajax({
                        type: "post",
                        ...
                        dataType: "json",
                        data: {k: key, u: getCookie('id'), password: password},
                        success: function (p) {
                            if (p.datas.error) {
                                $.sDialog({skin: "red", content: p.datas.error, okBtn: false, cancelBtn: false});
                                return false
