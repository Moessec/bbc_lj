        ...
        $.ajax({
            type: "post", url: ApiUrl + "/index.php?ctl=Buyer_Bespeak&met=rentBespeak&typ=json", data: {k: key, u:getCookie('id')}, dataType: "json", success: function (e)
            {
                ...
                $.each(e.data.temp, function(key, value){
                       ...
                       tem+='<dd style="float: left; width:54%;line-height: 1.6rem;color:#000;font-size:0.8rem">'+value.bespeak_title+'</dd>';
                       tem+='<dd style="float: left; width:54%;line-height: 1rem;">归还时间：'+value.outtime+'</dd>'
                       tem+='<dd style="float: left; width:54%;line-height: 1rem;">价格：'+value.rent_price+'</dd>';
                       tem+='<dd style="float: left; width:54%;line-height: 1rem;">备注：'+value.bespeak_com+'</dd></div></dl></li></ul>';
