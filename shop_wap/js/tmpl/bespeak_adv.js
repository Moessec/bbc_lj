        ...
        $.ajax({
            type: "post", url: ApiUrl + "/index.php?ctl=Buyer_Bespeak&met=advBespeak&typ=json", data: {k: key, u:getCookie('id')}, dataType: "json", success: function (e)
            {
                ...
                $.each(e.data.adv, function(key, value){
                       tem='<ul><li><dl><dt><span class="name">'+value.bespeak_title+'</span></dt><img src="'+value.bespeak_img+'" style="width:50px;height:50px"><dd style="margin-left:20px">活动时间：'+value.opentime+'</br>--"'+value.outtime+'"</dd><dd><br/>地点：'+value.bespeak_address+'</dd>
