        ...
        $.ajax({
            type: "post", url: ApiUrl + "/index.php?ctl=Buyer_Bespeak&met=editBespeak&typ=json", data: {k: key, u:getCookie('id')}, dataType: "json", success: function (e)
            {
                ...
                $.each(e.data, function(key, value){
                        temp='<ul><li><dl><dt><span>联系人：'+value.true_name+'</span><span style="margin-left:10px;font-size: 0.6rem;color: #444;">联系方式：'+value.usercontact+'</span></dt><dd style="margin-top:10px;font-size: 0.6rem;color: #444;">报修项目：'+value.bespeak_place+'</dd><dd style="margin-top:10px;">报修地址：'+value.bespeak_address+'</dd><dd style="margin-top:10px;font-size: 0.6rem;color: #444;">报修详情：'+value.bespeak_com+'</dd><dd style="margin-top:10px;"><img src="http://139.196.51.206/bbc_lj/shop/shop/data'+value.bespeak_img+'" width="100px" height="100px" style="display: block; "></dd></dl><div class="handle">'+value.bespeak_state+'<span><a href="javascript:;" bespeak_id="'+value.bespeak_id+'" class="delbespeak"><i class="del"></i>删除</a></span></div></li></ul>';
