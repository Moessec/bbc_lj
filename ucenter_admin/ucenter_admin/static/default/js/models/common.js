...
//设置表格宽高
Public.setDialogGrid = function(adjustH, adjustW){

    var adjustH = adjustH || 65;
    var adjustW = adjustW || 35;

    var gridW = frameElement.api.config.width - adjustW, gridH = frameElement.api.config.height - $(".grid-wrap").offset().top - adjustH;

    $('.wrapper').css("min-width", gridW);
...
Public.operFmatter_new = function (val, opt, row) {
    var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-plus" title="新增行">&#xe605;</span><span class="ui-icon ui-icon-pencil" title="修改"></span><span class="ui-icon ui-icon-trash" title="删除"></span></div>';
    return html_con;
};
...
Business.customerCombo = function($_obj, opts){
    ...
    var opts = $.extend(true, {
        ...
        formatText: function(row){
            ...
            //return row.number + ' ' + row.customer_name;
        },
        //formatResult: 'name',
        text: 'name',
        value: 'id',
    ...
    $('#quickAddCustomer').on('click', function(e){
        ...
        $.dialog({
            title : '新增客户',
            content : 'url:./erp.php?ctl=Member_Base&met=manage',
...
Business.brandCombo = function($_obj, opts){
    ...
    var opts = $.extend(true, {
        data: function(){
            if(defaultPage.SYSTEM.brandInfo) {
                return defaultPage.SYSTEM.brandInfo;
            } else {
                //return SITE_URL + '?ctl=Goods_Brand&met=lists&typ=json';
                return SITE_URL + '?ctl=Category&met=lists&typ=json&type_number=brand';
            }
        },

        ajaxOptions: {
            formatData: function(data){
                defaultPage.SYSTEM.brandInfo = data.data.items;	//更新
        ...
        formatText: function(row){
            ...
            //return row.number + ' ' + row.brand_name;
        },
        //formatResult: 'name',
        text: 'brand_name',
        value: 'brand_id',
        ...
        callback: {
            ...
            onEnter:function(e){
                ...
                var skey = $_obj.find('input').val();
                if(!$_obj.data('contactInfo')){
                    $_obj.find('.ui-icon-ellipsis').data('skey',skey).data('type','brandInfo').trigger('click');
                }
            }
        }
    }, opts);

    var brandCombo = $_obj.combo(opts).getCombo();
    //新增客户
    $('#quickAddCustomer').on('click', function(e){
        e.preventDefault();
        if (!Business.verifyRight('BU_ADD')) {
        ...
        $.dialog({
            ...
            data: {oper: 'add', callback: function(data, oper, dialogWin){
                //parent.getCustomer();
                //_self.brandCombo.selectByValue(data.id, false);
                //brandCombo.loadData('/basedata/contact.do?action=list', ['id', data.id]);
                if(data && data.id) {
                    $_obj.data('contactInfo', data);	//存储
                    $_obj.find('input').val(data.number + ' ' + data.name);	//回填数据
                    defaultPage.SYSTEM.brandInfo.push(data);	//增加进缓存
                    brandCombo.collapse();	//关闭下拉
                }
                dialogWin && dialogWin.api.close();
            }},
            width : 640,
    ...
    brandCombo.input.focus(function() {
        var $_this = $(this);
        setTimeout(function(){
            $_this.select();
        }, 15);
...
Business.billSalesCombo = function($_obj, opts){
    ...
    opts = $.extend(true, {
        data: function(){
            if(defaultPage.SYSTEM.salesInfo) {
                ...
                return './erp.php?ctl=Base_Employee&met=employeeList&typ=json';
            }
        }
    }, opts);
  ...
  Business.salesCombo = function($_obj, opts){
    ...
    $('#quickAddSales').on('click', function(e){
        ...
        $.dialog({
            title : '新增职员',
            content : 'url:./erp.php?ctl=Base_Employee&met=manage',
            data: {oper: 'add', callback: function(data, oper, dialogWin){
                //parent.getCustomer();
                //_self.customerCombo.selectByValue(data.id, false);
                salesCombo.loadData('./erp.php?ctl=Base_Employee&met=employeeList', ['id', data.id]);
                dialogWin && dialogWin.api.close();
            }},
            width : 400,
            height : 160,
...
Business.supplierCombo = function($_obj, opts){
    ...
    var opts = $.extend(true, {
        ...
        height: 300,
        formatText: function(row){
            return row.vendor_number + ' ' + row.vendor_name;
        },
        //formatResult: 'name',
        text: 'name',
        value: 'id',
    ...
    $('#quickAddVendor').on('click', function(e){
        ...
        $.dialog({
            title : '新增供应商',
            content : 'url:./erp.php?ctl=Vendor_Base&met=manage',
            data: {oper: 'add', callback: function(data, oper, dialogWin){
                //parent.getCustomer();
                //_self.customerCombo.selectByValue(data.id, false);
...
Business.cacheManage = {
    ...
    getGoodInfo:function(params, callback, effective){
        ...
        switch(params.action){
            case 'queryByMatchCon':_prop = 'goods_number';val = params.matchCon; break;
        ...
        //本地匹配不到，则去后端查找
        $.ajax({
            url: './erp.php?ctl=Goods_Base&met=goodsList&isDelete=2&typ=json',
            type: 'post',
            dataType: 'json',
            data: params,
            async:false,//勿修改该属性
...
Business.serNumManage = function(opts){
    var parentTr = opts.row,
        data = opts.data || parentTr.data('goodsInfo') || {},
        serNumUsedList = data.serNumList,
    ...
    var params = {
        width: 650,
        height: 400,
        title: (serNumUsedList ? '修改' : '录入') + '【'+data.goods_name+'】的物资的位置编号',
        content: 'url:./erp.php?ctl=Goods_Base&met=sernum',
        data: {
            invId: data.id,
            creatable:creatable,
            ...
            callback: function(serNumList,win){
				...
				for (var i = 0; i < serNumList.length; i++) {
					...
					if(creatable && !$.isEmptyObject(oldData.skuInfo)){
						//新增的时候检查，不新增分录
                ...
                for(item in storageList){
                    ...
                    tempRowData.goods = (data.goods_number ? data.goods_number : data.number) + ' ' + data.goods_name + (data.spec ? '_' + data.spec: '');
                    //desc修改成数量
                    var qty = 0;
                    for(var k in serNums){
                    	qty += parseInt(serNums[k]['desc']);
                    }
                    tempRowData.qty = qty;
    ...
    if(enableStorage){
        ...
        params.title = '选择【'+data.goods_name+'】的序列号',
            params.content = 'url:./erp.php?ctl=Sale_Bill&met=serNumList';
    ...
    if(opts.type == 'sales')
    {
    	params.content = 'url:./erp.php?ctl=Goods_Base&met=sernum#type=sales';
    }
    if(view){
        //查询模式
        params.cancel = true;
...
Business.billSkuManage = function(parentTr , data, isSkuSingle){
    ...
    $.dialog({
        width: 470,
        height: 410,
        title: '选择【'+data.goods_name+'】的属性',
        content: 'url:./erp.php?ctl=Goods_Base&met=skus',
        data: {
            //skey:_self.skey,
            ...
            callback: function(goodsPropList,win){
                ...
                if(isSkuSingle){
                    ...
                    for(var i = 0 ,len = goodsPropList.length; i<len; i++){
                        ...
                        var tempRowData = $.extend(true, {}, data);//克隆对象，不然会污染combo的数据
                        tempRowData.goods = data.goods_number + ' ' + data.goods_name + (data.spec ? '_' + data.spec: '');
                        tempRowData.qty = tempData.qty;
                        tempRowData.skuName = tempData.skuName;
                        tempRowData.skuId = tempData.skuId;
...
Business.goodsCombo = function($_obj, opts){
    ...
    var opts = $.extend(true, {
        ...
        ajaxOptions: {
            formatData: function(data){
                defaultPage.SYSTEM.goodsInfo = data.data.items;	//更新
                return data.data.rows;
            }
        },
        ...
        callback: {
            ...
            incrementalSearch: function(pos, callback){
                var _self = this;
                var query = $_obj.val()
                Public.ajaxGet('', { rows: 20, skey: query }, function(data){
...
Business.categoryCombo = function($_obj, opts, type){
    ...
    var opts = $.extend(true, {
        data: function(){
            if(defaultPage.SYSTEM.categoryInfo && defaultPage.SYSTEM.categoryInfo[type_number]) {
                return defaultPage.SYSTEM.categoryInfo[type_number];
            } else {
				return SITE_URL + '?ctl=Category&met=lists&typ=json&type_number='+type_number;
            }
        },
        ajaxOptions: {
            formatData: function(data){
                defaultPage.SYSTEM.categoryInfo = defaultPage.SYSTEM.categoryInfo ||{};
                defaultPage.SYSTEM.categoryInfo[type_number] = data.data[type_number];	//更新
                return  data.data[type_number];
            }
        },
        text: 'name',
        value: 'id',
    ...
    $('#quickAddCategory').on('click', function(e){
        ...
        /*
        if (rights[type_number] && !Business.verifyRight(rights[type_number])) {
            return ;
        };
        */
        var callback=function(data,dialogWin){
            categoryCombo.loadData(function(){return defaultPage.SYSTEM.categoryInfo[type_number]}, '-1', false);
            dialogWin.close();
            setTimeout( function() {
        ...
        Public.categoryPop(type_number,window.parent,callback);
    });
...
Business.forSearch = function(id, text){
    if(id) {
        $.dialog({
            ...
            height: 410,
            title: '商品库存查询',
            //content: 'url:./erp.php?ctl=Goods_WarehouseSku&met=index',
            content: 'url:./erp.php?ctl=Goods_WarehouseSku&met=manage&goods_id=' + id,
...
Business.timeZoneCombo = function($_obj, opts){
    var defaultPage = Public.getDefaultPage();
    if ($_obj.length == 0) { return };
    var opts = $.extend(true, {
        data: function(){
            return  [{"id":"Pacific\/Midway","name":"(GMT-11:00) Pacific, Midway"},{"id":"Pacific\/Niue","name":"(GMT-11:00) Pacific, Niue"},{"id":"Pacific\/Pago_Pago","name":"(GMT-11:00) Pacific, Pago Pago"},{"id":"Pacific\/Honolulu","name":"(GMT-10:00) Pacific, Honolulu"},{"id":"Pacific\/Johnston","name":"(GMT-10:00) Pacific, Johnston"},{"id":"Pacific\/Rarotonga","name":"(GMT-10:00) Pacific, Rarotonga"},{"id":"Pacific\/Tahiti","name":"(GMT-10:00) Pacific, Tahiti"},{"id":"Pacific\/Marquesas","name":"(GMT-09:30) Pacific, Marquesas"},{"id":"America\/Adak","name":"(GMT-09:00) America, Adak"},{"id":"Pacific\/Gambier","name":"(GMT-09:00) Pacific, Gambier"},{"id":"America\/Anchorage","name":"(GMT-08:00) America, Anchorage"},{"id":"America\/Juneau","name":"(GMT-08:00) America, Juneau"},{"id":"America\/Metlakatla","name":"(GMT-08:00) America, Metlakatla"},{"id":"America\/Nome","name":"(GMT-08:00) America, Nome"},{"id":"America\/Sitka","name":"(GMT-08:00) America, Sitka"},{"id":"America\/Yakutat","name":"(GMT-08:00) America, Yakutat"},{"id":"Pacific\/Pitcairn","name":"(GMT-08:00) Pacific, Pitcairn"},{"id":"America\/Creston","name":"(GMT-07:00) America, Creston"},{"id":"America\/Dawson","name":"(GMT-07:00) America, Dawson"},{"id":"America\/Dawson_Creek","name":"(GMT-07:00) America, Dawson Creek"},{"id":"America\/Fort_Nelson","name":"(GMT-07:00) America, Fort Nelson"},{"id":"America\/Hermosillo","name":"(GMT-07:00) America, Hermosillo"},{"id":"America\/Los_Angeles","name":"(GMT-07:00) America, Los Angeles"},{"id":"America\/Phoenix","name":"(GMT-07:00) America, Phoenix"},{"id":"America\/Santa_Isabel","name":"(GMT-07:00) America, Santa Isabel"},{"id":"America\/Tijuana","name":"(GMT-07:00) America, Tijuana"},{"id":"America\/Vancouver","name":"(GMT-07:00) America, Vancouver"},{"id":"America\/Whitehorse","name":"(GMT-07:00) America, Whitehorse"},{"id":"America\/Belize","name":"(GMT-06:00) America, Belize"},{"id":"America\/Boise","name":"(GMT-06:00) America, Boise"},{"id":"America\/Cambridge_Bay","name":"(GMT-06:00) America, Cambridge Bay"},{"id":"America\/Chihuahua","name":"(GMT-06:00) America, Chihuahua"},{"id":"America\/Costa_Rica","name":"(GMT-06:00) America, Costa Rica"},{"id":"America\/Denver","name":"(GMT-06:00) America, Denver"},{"id":"America\/Edmonton","name":"(GMT-06:00) America, Edmonton"},{"id":"America\/El_Salvador","name":"(GMT-06:00) America, El Salvador"},{"id":"America\/Guatemala","name":"(GMT-06:00) America, Guatemala"},{"id":"America\/Inuvik","name":"(GMT-06:00) America, Inuvik"},{"id":"America\/Managua","name":"(GMT-06:00) America, Managua"},{"id":"America\/Mazatlan","name":"(GMT-06:00) America, Mazatlan"},{"id":"America\/Ojinaga","name":"(GMT-06:00) America, Ojinaga"},{"id":"America\/Regina","name":"(GMT-06:00) America, Regina"},{"id":"America\/Swift_Current","name":"(GMT-06:00) America, Swift Current"},{"id":"America\/Tegucigalpa","name":"(GMT-06:00) America, Tegucigalpa"},{"id":"America\/Yellowknife","name":"(GMT-06:00) America, Yellowknife"},{"id":"Pacific\/Galapagos","name":"(GMT-06:00) Pacific, Galapagos"},{"id":"America\/Atikokan","name":"(GMT-05:00) America, Atikokan"},{"id":"America\/Bahia_Banderas","name":"(GMT-05:00) America, Bahia Banderas"},{"id":"America\/Bogota","name":"(GMT-05:00) America, Bogota"},{"id":"America\/Cancun","name":"(GMT-05:00) America, Cancun"},{"id":"America\/Chicago","name":"(GMT-05:00) America, Chicago"},{"id":"America\/Eirunepe","name":"(GMT-05:00) America, Eirunepe"},{"id":"America\/Guayaquil","name":"(GMT-05:00) America, Guayaquil"},{"id":"America\/Indiana\/Knox","name":"(GMT-05:00) America, Indiana, Knox"},{"id":"America\/Indiana\/Tell_City","name":"(GMT-05:00) America, Indiana, Tell City"},{"id":"America\/Jamaica","name":"(GMT-05:00) America, Jamaica"},{"id":"America\/Lima","name":"(GMT-05:00) America, Lima"},{"id":"America\/Matamoros","name":"(GMT-05:00) America, Matamoros"},{"id":"America\/Menominee","name":"(GMT-05:00) America, Menominee"},{"id":"America\/Merida","name":"(GMT-05:00) America, Merida"},{"id":"America\/Mexico_City","name":"(GMT-05:00) America, Mexico City"},{"id":"America\/Monterrey","name":"(GMT-05:00) America, Monterrey"},{"id":"America\/North_Dakota\/Beulah","name":"(GMT-05:00) America, North Dakota, Beulah"},{"id":"America\/North_Dakota\/Center","name":"(GMT-05:00) America, North Dakota, Center"},{"id":"America\/North_Dakota\/New_Salem","name":"(GMT-05:00) America, North Dakota, New Salem"},{"id":"America\/Panama","name":"(GMT-05:00) America, Panama"},{"id":"America\/Rainy_River","name":"(GMT-05:00) America, Rainy River"},{"id":"America\/Rankin_Inlet","name":"(GMT-05:00) America, Rankin Inlet"},{"id":"America\/Resolute","name":"(GMT-05:00) America, Resolute"},{"id":"America\/Rio_Branco","name":"(GMT-05:00) America, Rio Branco"},{"id":"America\/Winnipeg","name":"(GMT-05:00) America, Winnipeg"},{"id":"Pacific\/Easter","name":"(GMT-05:00) Pacific, Easter"},{"id":"America\/Caracas","name":"(GMT-04:30) America, Caracas"},{"id":"America\/Anguilla","name":"(GMT-04:00) America, Anguilla"},{"id":"America\/Antigua","name":"(GMT-04:00) America, Antigua"},{"id":"America\/Aruba","name":"(GMT-04:00) America, Aruba"},{"id":"America\/Asuncion","name":"(GMT-04:00) America, Asuncion"},{"id":"America\/Barbados","name":"(GMT-04:00) America, Barbados"},{"id":"America\/Blanc-Sablon","name":"(GMT-04:00) America, Blanc-Sablon"},{"id":"America\/Boa_Vista","name":"(GMT-04:00) America, Boa Vista"},{"id":"America\/Campo_Grande","name":"(GMT-04:00) America, Campo Grande"},{"id":"America\/Cayman","name":"(GMT-04:00) America, Cayman"},{"id":"America\/Cuiaba","name":"(GMT-04:00) America, Cuiaba"},{"id":"America\/Curacao","name":"(GMT-04:00) America, Curacao"},{"id":"America\/Detroit","name":"(GMT-04:00) America, Detroit"},{"id":"America\/Dominica","name":"(GMT-04:00) America, Dominica"},{"id":"America\/Grand_Turk","name":"(GMT-04:00) America, Grand Turk"},{"id":"America\/Grenada","name":"(GMT-04:00) America, Grenada"},{"id":"America\/Guadeloupe","name":"(GMT-04:00) America, Guadeloupe"},{"id":"America\/Guyana","name":"(GMT-04:00) America, Guyana"},{"id":"America\/Havana","name":"(GMT-04:00) America, Havana"},{"id":"America\/Indiana\/Indianapolis","name":"(GMT-04:00) America, Indiana, Indianapolis"},{"id":"America\/Indiana\/Marengo","name":"(GMT-04:00) America, Indiana, Marengo"},{"id":"America\/Indiana\/Petersburg","name":"(GMT-04:00) America, Indiana, Petersburg"},{"id":"America\/Indiana\/Vevay","name":"(GMT-04:00) America, Indiana, Vevay"},{"id":"America\/Indiana\/Vincennes","name":"(GMT-04:00) America, Indiana, Vincennes"},{"id":"America\/Indiana\/Winamac","name":"(GMT-04:00) America, Indiana, Winamac"},{"id":"America\/Iqaluit","name":"(GMT-04:00) America, Iqaluit"},{"id":"America\/Kentucky\/Louisville","name":"(GMT-04:00) America, Kentucky, Louisville"},{"id":"America\/Kentucky\/Monticello","name":"(GMT-04:00) America, Kentucky, Monticello"},{"id":"America\/Kralendijk","name":"(GMT-04:00) America, Kralendijk"},{"id":"America\/La_Paz","name":"(GMT-04:00) America, La Paz"},{"id":"America\/Lower_Princes","name":"(GMT-04:00) America, Lower Princes"},{"id":"America\/Manaus","name":"(GMT-04:00) America, Manaus"},{"id":"America\/Marigot","name":"(GMT-04:00) America, Marigot"},{"id":"America\/Martinique","name":"(GMT-04:00) America, Martinique"},{"id":"America\/Montserrat","name":"(GMT-04:00) America, Montserrat"},{"id":"America\/Nassau","name":"(GMT-04:00) America, Nassau"},{"id":"America\/New_York","name":"(GMT-04:00) America, New York"},{"id":"America\/Nipigon","name":"(GMT-04:00) America, Nipigon"},{"id":"America\/Pangnirtung","name":"(GMT-04:00) America, Pangnirtung"},{"id":"America\/Port_of_Spain","name":"(GMT-04:00) America, Port of Spain"},{"id":"America\/Port-au-Prince","name":"(GMT-04:00) America, Port-au-Prince"},{"id":"America\/Porto_Velho","name":"(GMT-04:00) America, Porto Velho"},{"id":"America\/Puerto_Rico","name":"(GMT-04:00) America, Puerto Rico"},{"id":"America\/Santo_Domingo","name":"(GMT-04:00) America, Santo Domingo"},{"id":"America\/St_Barthelemy","name":"(GMT-04:00) America, St. Barthelemy"},{"id":"America\/St_Kitts","name":"(GMT-04:00) America, St. Kitts"},{"id":"America\/St_Lucia","name":"(GMT-04:00) America, St. Lucia"},{"id":"America\/St_Thomas","name":"(GMT-04:00) America, St. Thomas"},{"id":"America\/St_Vincent","name":"(GMT-04:00) America, St. Vincent"},{"id":"America\/Thunder_Bay","name":"(GMT-04:00) America, Thunder Bay"},{"id":"America\/Toronto","name":"(GMT-04:00) America, Toronto"},{"id":"America\/Tortola","name":"(GMT-04:00) America, Tortola"},{"id":"America\/Araguaina","name":"(GMT-03:00) America, Araguaina"},{"id":"America\/Argentina\/Buenos_Aires","name":"(GMT-03:00) America, Argentina, Buenos Aires"},{"id":"America\/Argentina\/Catamarca","name":"(GMT-03:00) America, Argentina, Catamarca"},{"id":"America\/Argentina\/Cordoba","name":"(GMT-03:00) America, Argentina, Cordoba"},{"id":"America\/Argentina\/Jujuy","name":"(GMT-03:00) America, Argentina, Jujuy"},{"id":"America\/Argentina\/La_Rioja","name":"(GMT-03:00) America, Argentina, La Rioja"},{"id":"America\/Argentina\/Mendoza","name":"(GMT-03:00) America, Argentina, Mendoza"},{"id":"America\/Argentina\/Rio_Gallegos","name":"(GMT-03:00) America, Argentina, Rio Gallegos"},{"id":"America\/Argentina\/Salta","name":"(GMT-03:00) America, Argentina, Salta"},{"id":"America\/Argentina\/San_Juan","name":"(GMT-03:00) America, Argentina, San Juan"},{"id":"America\/Argentina\/San_Luis","name":"(GMT-03:00) America, Argentina, San Luis"},{"id":"America\/Argentina\/Tucuman","name":"(GMT-03:00) America, Argentina, Tucuman"},{"id":"America\/Argentina\/Ushuaia","name":"(GMT-03:00) America, Argentina, Ushuaia"},{"id":"America\/Bahia","name":"(GMT-03:00) America, Bahia"},{"id":"America\/Belem","name":"(GMT-03:00) America, Belem"},{"id":"America\/Cayenne","name":"(GMT-03:00) America, Cayenne"},{"id":"America\/Fortaleza","name":"(GMT-03:00) America, Fortaleza"},{"id":"America\/Glace_Bay","name":"(GMT-03:00) America, Glace Bay"},{"id":"America\/Goose_Bay","name":"(GMT-03:00) America, Goose Bay"},{"id":"America\/Halifax","name":"(GMT-03:00) America, Halifax"},{"id":"America\/Maceio","name":"(GMT-03:00) America, Maceio"},{"id":"America\/Moncton","name":"(GMT-03:00) America, Moncton"},{"id":"America\/Montevideo","name":"(GMT-03:00) America, Montevideo"},{"id":"America\/Paramaribo","name":"(GMT-03:00) America, Paramaribo"},{"id":"America\/Recife","name":"(GMT-03:00) America, Recife"},{"id":"America\/Santarem","name":"(GMT-03:00) America, Santarem"},{"id":"America\/Santiago","name":"(GMT-03:00) America, Santiago"},{"id":"America\/Sao_Paulo","name":"(GMT-03:00) America, Sao Paulo"},{"id":"America\/Thule","name":"(GMT-03:00) America, Thule"},{"id":"Antarctica\/Palmer","name":"(GMT-03:00) Antarctica, Palmer"},{"id":"Antarctica\/Rothera","name":"(GMT-03:00) Antarctica, Rothera"},{"id":"Atlantic\/Bermuda","name":"(GMT-03:00) Atlantic, Bermuda"},{"id":"Atlantic\/Stanley","name":"(GMT-03:00) Atlantic, Stanley"},{"id":"America\/St_Johns","name":"(GMT-02:30) America, St. Johns"},{"id":"America\/Godthab","name":"(GMT-02:00) America, Godthab"},{"id":"America\/Miquelon","name":"(GMT-02:00) America, Miquelon"},{"id":"America\/Noronha","name":"(GMT-02:00) America, Noronha"},{"id":"Atlantic\/South_Georgia","name":"(GMT-02:00) Atlantic, South Georgia"},{"id":"Atlantic\/Cape_Verde","name":"(GMT-01:00) Atlantic, Cape Verde"},{"id":"Africa\/Abidjan","name":"(GMT) Africa, Abidjan"},{"id":"Africa\/Accra","name":"(GMT) Africa, Accra"},{"id":"Africa\/Bamako","name":"(GMT) Africa, Bamako"},{"id":"Africa\/Banjul","name":"(GMT) Africa, Banjul"},{"id":"Africa\/Bissau","name":"(GMT) Africa, Bissau"},{"id":"Africa\/Casablanca","name":"(GMT) Africa, Casablanca"},{"id":"Africa\/Conakry","name":"(GMT) Africa, Conakry"},{"id":"Africa\/Dakar","name":"(GMT) Africa, Dakar"},{"id":"Africa\/El_Aaiun","name":"(GMT) Africa, El Aaiun"},{"id":"Africa\/Freetown","name":"(GMT) Africa, Freetown"},{"id":"Africa\/Lome","name":"(GMT) Africa, Lome"},{"id":"Africa\/Monrovia","name":"(GMT) Africa, Monrovia"},{"id":"Africa\/Nouakchott","name":"(GMT) Africa, Nouakchott"},{"id":"Africa\/Ouagadougou","name":"(GMT) Africa, Ouagadougou"},{"id":"Africa\/Sao_Tome","name":"(GMT) Africa, Sao Tome"},{"id":"America\/Danmarkshavn","name":"(GMT) America, Danmarkshavn"},{"id":"America\/Scoresbysund","name":"(GMT) America, Scoresbysund"},{"id":"Atlantic\/Azores","name":"(GMT) Atlantic, Azores"},{"id":"Atlantic\/Reykjavik","name":"(GMT) Atlantic, Reykjavik"},{"id":"Atlantic\/St_Helena","name":"(GMT) Atlantic, St. Helena"},{"id":"UTC","name":"(GMT) UTC"},{"id":"Africa\/Algiers","name":"(GMT+01:00) Africa, Algiers"},{"id":"Africa\/Bangui","name":"(GMT+01:00) Africa, Bangui"},{"id":"Africa\/Brazzaville","name":"(GMT+01:00) Africa, Brazzaville"},{"id":"Africa\/Douala","name":"(GMT+01:00) Africa, Douala"},{"id":"Africa\/Kinshasa","name":"(GMT+01:00) Africa, Kinshasa"},{"id":"Africa\/Lagos","name":"(GMT+01:00) Africa, Lagos"},{"id":"Africa\/Libreville","name":"(GMT+01:00) Africa, Libreville"},{"id":"Africa\/Luanda","name":"(GMT+01:00) Africa, Luanda"},{"id":"Africa\/Malabo","name":"(GMT+01:00) Africa, Malabo"},{"id":"Africa\/Ndjamena","name":"(GMT+01:00) Africa, Ndjamena"},{"id":"Africa\/Niamey","name":"(GMT+01:00) Africa, Niamey"},{"id":"Africa\/Porto-Novo","name":"(GMT+01:00) Africa, Porto-Novo"},{"id":"Africa\/Tunis","name":"(GMT+01:00) Africa, Tunis"},{"id":"Africa\/Windhoek","name":"(GMT+01:00) Africa, Windhoek"},{"id":"Atlantic\/Canary","name":"(GMT+01:00) Atlantic, Canary"},{"id":"Atlantic\/Faroe","name":"(GMT+01:00) Atlantic, Faroe"},{"id":"Atlantic\/Madeira","name":"(GMT+01:00) Atlantic, Madeira"},{"id":"Europe\/Dublin","name":"(GMT+01:00) Europe, Dublin"},{"id":"Europe\/Guernsey","name":"(GMT+01:00) Europe, Guernsey"},{"id":"Europe\/Isle_of_Man","name":"(GMT+01:00) Europe, Isle of Man"},{"id":"Europe\/Jersey","name":"(GMT+01:00) Europe, Jersey"},{"id":"Europe\/Lisbon","name":"(GMT+01:00) Europe, Lisbon"},{"id":"Europe\/London","name":"(GMT+01:00) Europe, London"},{"id":"Africa\/Blantyre","name":"(GMT+02:00) Africa, Blantyre"},{"id":"Africa\/Bujumbura","name":"(GMT+02:00) Africa, Bujumbura"},{"id":"Africa\/Cairo","name":"(GMT+02:00) Africa, Cairo"},{"id":"Africa\/Ceuta","name":"(GMT+02:00) Africa, Ceuta"},{"id":"Africa\/Gaborone","name":"(GMT+02:00) Africa, Gaborone"},{"id":"Africa\/Harare","name":"(GMT+02:00) Africa, Harare"},{"id":"Africa\/Johannesburg","name":"(GMT+02:00) Africa, Johannesburg"},{"id":"Africa\/Kigali","name":"(GMT+02:00) Africa, Kigali"},{"id":"Africa\/Lubumbashi","name":"(GMT+02:00) Africa, Lubumbashi"},{"id":"Africa\/Lusaka","name":"(GMT+02:00) Africa, Lusaka"},{"id":"Africa\/Maputo","name":"(GMT+02:00) Africa, Maputo"},{"id":"Africa\/Maseru","name":"(GMT+02:00) Africa, Maseru"},{"id":"Africa\/Mbabane","name":"(GMT+02:00) Africa, Mbabane"},{"id":"Africa\/Tripoli","name":"(GMT+02:00) Africa, Tripoli"},{"id":"Antarctica\/Troll","name":"(GMT+02:00) Antarctica, Troll"},{"id":"Arctic\/Longyearbyen","name":"(GMT+02:00) Arctic, Longyearbyen"},{"id":"Europe\/Amsterdam","name":"(GMT+02:00) Europe, Amsterdam"},{"id":"Europe\/Andorra","name":"(GMT+02:00) Europe, Andorra"},{"id":"Europe\/Belgrade","name":"(GMT+02:00) Europe, Belgrade"},{"id":"Europe\/Berlin","name":"(GMT+02:00) Europe, Berlin"},{"id":"Europe\/Bratislava","name":"(GMT+02:00) Europe, Bratislava"},{"id":"Europe\/Brussels","name":"(GMT+02:00) Europe, Brussels"},{"id":"Europe\/Budapest","name":"(GMT+02:00) Europe, Budapest"},{"id":"Europe\/Busingen","name":"(GMT+02:00) Europe, Busingen"},{"id":"Europe\/Copenhagen","name":"(GMT+02:00) Europe, Copenhagen"},{"id":"Europe\/Gibraltar","name":"(GMT+02:00) Europe, Gibraltar"},{"id":"Europe\/Kaliningrad","name":"(GMT+02:00) Europe, Kaliningrad"},{"id":"Europe\/Ljubljana","name":"(GMT+02:00) Europe, Ljubljana"},{"id":"Europe\/Luxembourg","name":"(GMT+02:00) Europe, Luxembourg"},{"id":"Europe\/Madrid","name":"(GMT+02:00) Europe, Madrid"},{"id":"Europe\/Malta","name":"(GMT+02:00) Europe, Malta"},{"id":"Europe\/Monaco","name":"(GMT+02:00) Europe, Monaco"},{"id":"Europe\/Oslo","name":"(GMT+02:00) Europe, Oslo"},{"id":"Europe\/Paris","name":"(GMT+02:00) Europe, Paris"},{"id":"Europe\/Podgorica","name":"(GMT+02:00) Europe, Podgorica"},{"id":"Europe\/Prague","name":"(GMT+02:00) Europe, Prague"},{"id":"Europe\/Rome","name":"(GMT+02:00) Europe, Rome"},{"id":"Europe\/San_Marino","name":"(GMT+02:00) Europe, San Marino"},{"id":"Europe\/Sarajevo","name":"(GMT+02:00) Europe, Sarajevo"},{"id":"Europe\/Skopje","name":"(GMT+02:00) Europe, Skopje"},{"id":"Europe\/Stockholm","name":"(GMT+02:00) Europe, Stockholm"},{"id":"Europe\/Tirane","name":"(GMT+02:00) Europe, Tirane"},{"id":"Europe\/Vaduz","name":"(GMT+02:00) Europe, Vaduz"},{"id":"Europe\/Vatican","name":"(GMT+02:00) Europe, Vatican"},{"id":"Europe\/Vienna","name":"(GMT+02:00) Europe, Vienna"},{"id":"Europe\/Warsaw","name":"(GMT+02:00) Europe, Warsaw"},{"id":"Europe\/Zagreb","name":"(GMT+02:00) Europe, Zagreb"},{"id":"Europe\/Zurich","name":"(GMT+02:00) Europe, Zurich"},{"id":"Africa\/Addis_Ababa","name":"(GMT+03:00) Africa, Addis Ababa"},{"id":"Africa\/Asmara","name":"(GMT+03:00) Africa, Asmara"},{"id":"Africa\/Dar_es_Salaam","name":"(GMT+03:00) Africa, Dar es Salaam"},{"id":"Africa\/Djibouti","name":"(GMT+03:00) Africa, Djibouti"},{"id":"Africa\/Juba","name":"(GMT+03:00) Africa, Juba"},{"id":"Africa\/Kampala","name":"(GMT+03:00) Africa, Kampala"},{"id":"Africa\/Khartoum","name":"(GMT+03:00) Africa, Khartoum"},{"id":"Africa\/Mogadishu","name":"(GMT+03:00) Africa, Mogadishu"},{"id":"Africa\/Nairobi","name":"(GMT+03:00) Africa, Nairobi"},{"id":"Antarctica\/Syowa","name":"(GMT+03:00) Antarctica, Syowa"},{"id":"Asia\/Aden","name":"(GMT+03:00) Asia, Aden"},{"id":"Asia\/Amman","name":"(GMT+03:00) Asia, Amman"},{"id":"Asia\/Baghdad","name":"(GMT+03:00) Asia, Baghdad"},{"id":"Asia\/Bahrain","name":"(GMT+03:00) Asia, Bahrain"},{"id":"Asia\/Beirut","name":"(GMT+03:00) Asia, Beirut"},{"id":"Asia\/Damascus","name":"(GMT+03:00) Asia, Damascus"},{"id":"Asia\/Gaza","name":"(GMT+03:00) Asia, Gaza"},{"id":"Asia\/Hebron","name":"(GMT+03:00) Asia, Hebron"},{"id":"Asia\/Jerusalem","name":"(GMT+03:00) Asia, Jerusalem"},{"id":"Asia\/Kuwait","name":"(GMT+03:00) Asia, Kuwait"},{"id":"Asia\/Nicosia","name":"(GMT+03:00) Asia, Nicosia"},{"id":"Asia\/Qatar","name":"(GMT+03:00) Asia, Qatar"},{"id":"Asia\/Riyadh","name":"(GMT+03:00) Asia, Riyadh"},{"id":"Europe\/Athens","name":"(GMT+03:00) Europe, Athens"},{"id":"Europe\/Bucharest","name":"(GMT+03:00) Europe, Bucharest"},{"id":"Europe\/Chisinau","name":"(GMT+03:00) Europe, Chisinau"},{"id":"Europe\/Helsinki","name":"(GMT+03:00) Europe, Helsinki"},{"id":"Europe\/Istanbul","name":"(GMT+03:00) Europe, Istanbul"},{"id":"Europe\/Kiev","name":"(GMT+03:00) Europe, Kiev"},{"id":"Europe\/Mariehamn","name":"(GMT+03:00) Europe, Mariehamn"},{"id":"Europe\/Minsk","name":"(GMT+03:00) Europe, Minsk"},{"id":"Europe\/Moscow","name":"(GMT+03:00) Europe, Moscow"},{"id":"Europe\/Riga","name":"(GMT+03:00) Europe, Riga"},{"id":"Europe\/Simferopol","name":"(GMT+03:00) Europe, Simferopol"},{"id":"Europe\/Sofia","name":"(GMT+03:00) Europe, Sofia"},{"id":"Europe\/Tallinn","name":"(GMT+03:00) Europe, Tallinn"},{"id":"Europe\/Uzhgorod","name":"(GMT+03:00) Europe, Uzhgorod"},{"id":"Europe\/Vilnius","name":"(GMT+03:00) Europe, Vilnius"},{"id":"Europe\/Volgograd","name":"(GMT+03:00) Europe, Volgograd"},{"id":"Europe\/Zaporozhye","name":"(GMT+03:00) Europe, Zaporozhye"},{"id":"Indian\/Antananarivo","name":"(GMT+03:00) Indian, Antananarivo"},{"id":"Indian\/Comoro","name":"(GMT+03:00) Indian, Comoro"},{"id":"Indian\/Mayotte","name":"(GMT+03:00) Indian, Mayotte"},{"id":"Asia\/Dubai","name":"(GMT+04:00) Asia, Dubai"},{"id":"Asia\/Muscat","name":"(GMT+04:00) Asia, Muscat"},{"id":"Asia\/Tbilisi","name":"(GMT+04:00) Asia, Tbilisi"},{"id":"Asia\/Yerevan","name":"(GMT+04:00) Asia, Yerevan"},{"id":"Europe\/Samara","name":"(GMT+04:00) Europe, Samara"},{"id":"Indian\/Mahe","name":"(GMT+04:00) Indian, Mahe"},{"id":"Indian\/Mauritius","name":"(GMT+04:00) Indian, Mauritius"},{"id":"Indian\/Reunion","name":"(GMT+04:00) Indian, Reunion"},{"id":"Asia\/Kabul","name":"(GMT+04:30) Asia, Kabul"},{"id":"Asia\/Tehran","name":"(GMT+04:30) Asia, Tehran"},{"id":"Antarctica\/Mawson","name":"(GMT+05:00) Antarctica, Mawson"},{"id":"Asia\/Aqtau","name":"(GMT+05:00) Asia, Aqtau"},{"id":"Asia\/Aqtobe","name":"(GMT+05:00) Asia, Aqtobe"},{"id":"Asia\/Ashgabat","name":"(GMT+05:00) Asia, Ashgabat"},{"id":"Asia\/Baku","name":"(GMT+05:00) Asia, Baku"},{"id":"Asia\/Dushanbe","name":"(GMT+05:00) Asia, Dushanbe"},{"id":"Asia\/Karachi","name":"(GMT+05:00) Asia, Karachi"},{"id":"Asia\/Oral","name":"(GMT+05:00) Asia, Oral"},{"id":"Asia\/Samarkand","name":"(GMT+05:00) Asia, Samarkand"},{"id":"Asia\/Tashkent","name":"(GMT+05:00) Asia, Tashkent"},{"id":"Asia\/Yekaterinburg","name":"(GMT+05:00) Asia, Yekaterinburg"},{"id":"Indian\/Kerguelen","name":"(GMT+05:00) Indian, Kerguelen"},{"id":"Indian\/Maldives","name":"(GMT+05:00) Indian, Maldives"},{"id":"Asia\/Colombo","name":"(GMT+05:30) Asia, Colombo"},{"id":"Asia\/Kolkata","name":"(GMT+05:30) Asia, Kolkata"},{"id":"Asia\/Kathmandu","name":"(GMT+05:45) Asia, Kathmandu"},{"id":"Antarctica\/Vostok","name":"(GMT+06:00) Antarctica, Vostok"},{"id":"Asia\/Almaty","name":"(GMT+06:00) Asia, Almaty"},{"id":"Asia\/Bishkek","name":"(GMT+06:00) Asia, Bishkek"},{"id":"Asia\/Dhaka","name":"(GMT+06:00) Asia, Dhaka"},{"id":"Asia\/Novosibirsk","name":"(GMT+06:00) Asia, Novosibirsk"},{"id":"Asia\/Omsk","name":"(GMT+06:00) Asia, Omsk"},{"id":"Asia\/Qyzylorda","name":"(GMT+06:00) Asia, Qyzylorda"},{"id":"Asia\/Thimphu","name":"(GMT+06:00) Asia, Thimphu"},{"id":"Asia\/Urumqi","name":"(GMT+06:00) Asia, Urumqi"},{"id":"Indian\/Chagos","name":"(GMT+06:00) Indian, Chagos"},{"id":"Asia\/Rangoon","name":"(GMT+06:30) Asia, Rangoon"},{"id":"Indian\/Cocos","name":"(GMT+06:30) Indian, Cocos"},{"id":"Antarctica\/Davis","name":"(GMT+07:00) Antarctica, Davis"},{"id":"Asia\/Bangkok","name":"(GMT+07:00) Asia, Bangkok"},{"id":"Asia\/Ho_Chi_Minh","name":"(GMT+07:00) Asia, Ho Chi Minh"},{"id":"Asia\/Jakarta","name":"(GMT+07:00) Asia, Jakarta"},{"id":"Asia\/Krasnoyarsk","name":"(GMT+07:00) Asia, Krasnoyarsk"},{"id":"Asia\/Novokuznetsk","name":"(GMT+07:00) Asia, Novokuznetsk"},{"id":"Asia\/Phnom_Penh","name":"(GMT+07:00) Asia, Phnom Penh"},{"id":"Asia\/Pontianak","name":"(GMT+07:00) Asia, Pontianak"},{"id":"Asia\/Vientiane","name":"(GMT+07:00) Asia, Vientiane"},{"id":"Indian\/Christmas","name":"(GMT+07:00) Indian, Christmas"},{"id":"Antarctica\/Casey","name":"(GMT+08:00) Antarctica, Casey"},{"id":"Asia\/Brunei","name":"(GMT+08:00) Asia, Brunei"},{"id":"Asia\/Chita","name":"(GMT+08:00) Asia, Chita"},{"id":"Asia\/Hong_Kong","name":"(GMT+08:00) Asia, Hong Kong"},{"id":"Asia\/Hovd","name":"(GMT+08:00) Asia, Hovd"},{"id":"Asia\/Irkutsk","name":"(GMT+08:00) Asia, Irkutsk"},{"id":"Asia\/Kuala_Lumpur","name":"(GMT+08:00) Asia, Kuala Lumpur"},{"id":"Asia\/Kuching","name":"(GMT+08:00) Asia, Kuching"},{"id":"Asia\/Macau","name":"(GMT+08:00) Asia, Macau"},{"id":"Asia\/Makassar","name":"(GMT+08:00) Asia, Makassar"},{"id":"Asia\/Manila","name":"(GMT+08:00) Asia, Manila"},{"id":"Asia\/Shanghai","name":"(GMT+08:00) Asia, Shanghai"},{"id":"Asia\/Singapore","name":"(GMT+08:00) Asia, Singapore"},{"id":"Asia\/Taipei","name":"(GMT+08:00) Asia, Taipei"},{"id":"Australia\/Perth","name":"(GMT+08:00) Australia, Perth"},{"id":"Asia\/Pyongyang","name":"(GMT+08:30) Asia, Pyongyang"},{"id":"Australia\/Eucla","name":"(GMT+08:45) Australia, Eucla"},{"id":"Asia\/Choibalsan","name":"(GMT+09:00) Asia, Choibalsan"},{"id":"Asia\/Dili","name":"(GMT+09:00) Asia, Dili"},{"id":"Asia\/Jayapura","name":"(GMT+09:00) Asia, Jayapura"},{"id":"Asia\/Khandyga","name":"(GMT+09:00) Asia, Khandyga"},{"id":"Asia\/Seoul","name":"(GMT+09:00) Asia, Seoul"},{"id":"Asia\/Tokyo","name":"(GMT+09:00) Asia, Tokyo"},{"id":"Asia\/Ulaanbaatar","name":"(GMT+09:00) Asia, Ulaanbaatar"},{"id":"Asia\/Yakutsk","name":"(GMT+09:00) Asia, Yakutsk"},{"id":"Pacific\/Palau","name":"(GMT+09:00) Pacific, Palau"},{"id":"Australia\/Adelaide","name":"(GMT+09:30) Australia, Adelaide"},{"id":"Australia\/Broken_Hill","name":"(GMT+09:30) Australia, Broken Hill"},{"id":"Australia\/Darwin","name":"(GMT+09:30) Australia, Darwin"},{"id":"Antarctica\/DumontDUrville","name":"(GMT+10:00) Antarctica, DumontDUrville"},{"id":"Asia\/Magadan","name":"(GMT+10:00) Asia, Magadan"},{"id":"Asia\/Sakhalin","name":"(GMT+10:00) Asia, Sakhalin"},{"id":"Asia\/Ust-Nera","name":"(GMT+10:00) Asia, Ust-Nera"},{"id":"Asia\/Vladivostok","name":"(GMT+10:00) Asia, Vladivostok"},{"id":"Australia\/Brisbane","name":"(GMT+10:00) Australia, Brisbane"},{"id":"Australia\/Currie","name":"(GMT+10:00) Australia, Currie"},{"id":"Australia\/Hobart","name":"(GMT+10:00) Australia, Hobart"},{"id":"Australia\/Lindeman","name":"(GMT+10:00) Australia, Lindeman"},{"id":"Australia\/Melbourne","name":"(GMT+10:00) Australia, Melbourne"},{"id":"Australia\/Sydney","name":"(GMT+10:00) Australia, Sydney"},{"id":"Pacific\/Chuuk","name":"(GMT+10:00) Pacific, Chuuk"},{"id":"Pacific\/Guam","name":"(GMT+10:00) Pacific, Guam"},{"id":"Pacific\/Port_Moresby","name":"(GMT+10:00) Pacific, Port Moresby"},{"id":"Pacific\/Saipan","name":"(GMT+10:00) Pacific, Saipan"},{"id":"Australia\/Lord_Howe","name":"(GMT+10:30) Australia, Lord Howe"},{"id":"Antarctica\/Macquarie","name":"(GMT+11:00) Antarctica, Macquarie"},{"id":"Asia\/Srednekolymsk","name":"(GMT+11:00) Asia, Srednekolymsk"},{"id":"Pacific\/Bougainville","name":"(GMT+11:00) Pacific, Bougainville"},{"id":"Pacific\/Efate","name":"(GMT+11:00) Pacific, Efate"},{"id":"Pacific\/Guadalcanal","name":"(GMT+11:00) Pacific, Guadalcanal"},{"id":"Pacific\/Kosrae","name":"(GMT+11:00) Pacific, Kosrae"},{"id":"Pacific\/Norfolk","name":"(GMT+11:00) Pacific, Norfolk"},{"id":"Pacific\/Noumea","name":"(GMT+11:00) Pacific, Noumea"},{"id":"Pacific\/Pohnpei","name":"(GMT+11:00) Pacific, Pohnpei"},{"id":"Antarctica\/McMurdo","name":"(GMT+12:00) Antarctica, McMurdo"},{"id":"Asia\/Anadyr","name":"(GMT+12:00) Asia, Anadyr"},{"id":"Asia\/Kamchatka","name":"(GMT+12:00) Asia, Kamchatka"},{"id":"Pacific\/Auckland","name":"(GMT+12:00) Pacific, Auckland"},{"id":"Pacific\/Fiji","name":"(GMT+12:00) Pacific, Fiji"},{"id":"Pacific\/Funafuti","name":"(GMT+12:00) Pacific, Funafuti"},{"id":"Pacific\/Kwajalein","name":"(GMT+12:00) Pacific, Kwajalein"},{"id":"Pacific\/Majuro","name":"(GMT+12:00) Pacific, Majuro"},{"id":"Pacific\/Nauru","name":"(GMT+12:00) Pacific, Nauru"},{"id":"Pacific\/Tarawa","name":"(GMT+12:00) Pacific, Tarawa"},{"id":"Pacific\/Wake","name":"(GMT+12:00) Pacific, Wake"},{"id":"Pacific\/Wallis","name":"(GMT+12:00) Pacific, Wallis"},{"id":"Pacific\/Chatham","name":"(GMT+12:45) Pacific, Chatham"},{"id":"Pacific\/Apia","name":"(GMT+13:00) Pacific, Apia"},{"id":"Pacific\/Enderbury","name":"(GMT+13:00) Pacific, Enderbury"},{"id":"Pacific\/Fakaofo","name":"(GMT+13:00) Pacific, Fakaofo"},{"id":"Pacific\/Tongatapu","name":"(GMT+13:00) Pacific, Tongatapu"},{"id":"Pacific\/Kiritimati","name":"(GMT+14:00) Pacific, Kiritimati"}];
        },
        text: 'name',
        value: 'id',
        defaultSelected: 0,
...
Business.languageCombo = function($_obj, opts){
    var defaultPage = Public.getDefaultPage();
    if ($_obj.length == 0) { return };
    var opts = $.extend(true, {
        data: function(){
...
Business.themeCombo = function($_obj, opts){
    var defaultPage = Public.getDefaultPage();
    if ($_obj.length == 0) { return };
    var opts = $.extend(true, {
        data: function(){
...
Business.propertyFormatCombo = function($_obj, opts){
    var defaultPage = Public.getDefaultPage();
    if ($_obj.length == 0) { return };
    var opts = $.extend(true, {
        data: function(){
            return property_format_row;
        },
        text: 'name',
        value: 'id',
        defaultSelected: 0,
...
//店铺等级
Business.gradeCombo = function($_obj, opts){
    var defaultPage = Public.getDefaultPage();
    if ($_obj.length == 0) { return };
    var opts = $.extend(true, {
        data: function(){
...
//店铺分类
Business.classCombo = function($_obj, opts){
    var defaultPage = Public.getDefaultPage();
    if ($_obj.length == 0) { return };
    var opts = $.extend(true, {
        data: function(){
    ...
    var opts = $.extend(true, {
        data: function(){
            if(SYSTEM.paymentInfo) {
                ...
                return './erp.php?ctl=Base_SettlementAccountType&met=SettlementAccountTypeLists&typ=json';
            }
        },
        ajaxOptions: {
            formatData: function(data){
    ...
    var opts = $.extend(true, {
        data: function(){
            if(SYSTEM.storeInfo) {
                ...
                return './erp.php?ctl=Store_Base&met=baseList&typ=json';
            }
        },
        ajaxOptions: {
            formatData: function(data){
    ...
    var opts = $.extend(true, {
        data: function(){
            if(SYSTEM.logisticInfo) {
                ...
                return './erp.php?ctl=Express_Base&met=baseList&typ=json';
            }
        },
        ajaxOptions: {
            formatData: function(data){
...
Business.billsEvent = function(obj, type, flag){
    ...
    if(type !== 'assemble') {
        $('#customer').on('click', '.ui-icon-ellipsis', function(e){
            ...
            if(lable === '供应商' || lable === '购货单位') {
                var content = 'url:./erp.php?ctl=Customer_Base&met=select&type=10';
            } else {
                var content = 'url:./erp.php?ctl=Customer_Base&met=select';
            }
        ...
        $('.grid-wrap').on('click', '.goods .ui-icon-ellipsis', function(e){
            ...
            $.dialog({
                ...
                content: 'url:./erp.php?ctl=Goods_Base&met=select',
    ...
    function initStorage() {
        ...
        for(var i = 0, len = data.length; i < data.length; i++) {
            list += '<li data-id="' + data[i].id + '" data-name="' + data[i].name + '" >' + data[i].warehouse_number + ' ' +data[i].name + '</li>';
        };
        list += '</ul>';
...
Business.filterCustomer = function(){
    Business.customerCombo($('#customerAuto'), {
        ...
        callback: {
            onChange: function(data){
                if(data) {
                    this.input.data('id', data.number);
...
Business.filterBrand = function(){
    Business.brandCombo($('#brand_id'), {
        width: '',
        formatText: function(data){
            return data.brand_id + ' ' + data.brand_name;
        },
        trigger: false,
        forceSelection: false,
        noDataText: '',
        ...
        callback: {
            onChange: function(data){
                if(data) {
                    this.input.data('id', data.brand_id);
                    this.input.val(data.brand_name);
                }
            }
        }
    });
    ...
    //客户
    $('#filter-brand .ui-icon-ellipsis').on('click', function(){
        var $input = $(this).prev('input');
        $.dialog({
            title: '选择品牌',
            content: 'url:' + SITE_URL + '?ctl=Goods_Brand&met=brandlist',
            data:{callback:function(a){$("#brand_id").data('id',a)},isDelete:2},
            lock: true,
            ok: function(){
                if(typeof this.content.callback === 'function'){
                    this.content.callback();
...
Public.print = function (opt){
    ...
    $.dialog({
        title: opt.title,
        content : 'url:./erp.php?ctl=PrintSettings',
        ...
        init:function(){
            ...
            var dialog = myParent.$.dialog({
                ...
                ok:function(){
                    ...
                    var oper = 'add';
                    var params = { name: category ,type_number: type };
                    var msg = '新增类别';
                   // alert(callback);
                    Public.ajaxPost('./erp.php?ctl=Finance_OtherType&met=income_add&income_type_name=' + params.name,params, function(data){
                        if (data.status == 200) {
...
var mod_AreasCombo = (function(mod) {
    ...
    mod.init = function(_provinceCombo,_cityCombo,_areaCombo ,callback){
		...
		Public.ajaxPost('./ucenter_admin/static/default/js/controllers/areas_data.js', {}, function(data){
            if(data) {
                _capable = true;
...
$(document).ready(function(){
    var off=false;
    $(".warn_xiaoma").click(function(){
        if(off==true){
            $(this).css("right","1px");
            $(this).next().css({"opacity":"1", "transition":"2s","display":"block"});
            $(this).find("span").css({"opacity":"1","transition":"2s","display":"block"});
            off=false;
        }else{
            $(this).next().css({"opacity":"0", "transition":"0.5s","display":"none"});
            $(this).find("span").css({
                "opacity":"0",
                "display":"none",
                "transition":"0.5s"
    ...
    $(".close_warn").click(function(){
            $(this).parent().parent().css({"opacity":"0", "transition":"0.5s","display":"none"});
            $(this).parent().parent().prev().find("span").css({
                    "opacity":"0",
                    "display":"none",
                    "transition":"0.5s"
                })
            $(this).parent().parent().prev().css("right","-69px");
            off=true; 
    })
})
