...
class IndexCtl extends Yf_AppController
{
	...
	public function db()
	{
		...
		$db_row = current($db_cfg['db_cfg_rows']['master']['ucenter_admin']);
		include $this->view->getView();
	}
	...
	public function install()
	{
		...
		else if (2 == $state_row['state'])
		{
			...
			foreach ($dir as $item)
			{
				...
				if (is_file($file))
				{
					$flag = $db->import($file, TABEL_PREFIX, 'ucenter_admin_');
	...
	public function createAdminAccount()
	{
		...
		*/
		...
		if (200 == $init_rs['status'])
		{
			...
			if ($User_BaseModel->getUser($data['user_id']))
			{
				$user_id                    = $data['user_id'];
			}
			else
			{
				$user_id                    = $User_BaseModel->addUser($data, true);
			}
...
function check_install_db()
{
	...
	if ($db_row = current($db_cfg['db_cfg_rows']['master']['ucenter_admin']))
	{
		try
		{
			$db_id = 'ucenter_admin';
