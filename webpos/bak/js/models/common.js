    ...
    opts = $.extend(true, {
        data: function(){
            if(defaultPage.SYSTEM.customerInfo) {
                ...
                return './member_select.php?action=getCustomer';
            }
        }
    }, opts);
    ...
    var opts = $.extend(true, {
        ...
        callback: {
            ...
            onEnter:function(e){
				...
				var url = "./member_select.php?action=getCustomer";
				var pars = 'skey='+skey;
				$.post(url, pars,showResponse);
				function showResponse(originalRequest)
				{
					$(".medium_brand").html(originalRequest);
				} */
    ...
    $('#quickAddSales').on('click', function(e){
        ...
        $.dialog({
            ...
            data: {oper: 'add', callback: function(data, oper, dialogWin){
                //parent.getCustomer();
                //_self.customerCombo.selectByValue(data.id, false);
                salesCombo.loadData('./sales.php?met=employee', ['id', data.id]);
                dialogWin && dialogWin.api.close();
            }},
            width : 400,
            height : 160,
    ...
    var opts = $.extend(true, {
        data: function(){
            if(defaultPage.SYSTEM.categoryInfo && defaultPage.SYSTEM.categoryInfo[typeNumber]) {
				...
				return './erp.php?ctl=Base_Assist&met=assistList&typeNumber=' + typeNumber;
            }
        },
        ajaxOptions: {
            formatData: function(data){
    ...
    if(type !== 'assemble') {
        $('#customer').on('click', '.ui-icon-ellipsis', function(e){
            ...
            if(lable === '供应商' || lable === '购货单位') {
                ...
                var content = 'url:./member_select.php';
            }
...
Public.print = function (opt){
    ...
    $.dialog({
        title: opt.title,
        content : 'url:./print.php?id='+originalData.id,
...
Public.zTree = {
    ...
    init: function($target, opts, setting ,callback) {
        ...
        Public.ajaxPost(opts.url || './product_select.php?op=cat', {}, function(data) {
            if (data.status === 200 && data.data) {
...
var mod_AreasCombo = (function(mod) {
    ...
    mod.init = function(_provinceCombo,_cityCombo,_areaCombo ,callback){
		...
		Public.ajaxPost('./erp/static/default/js/controllers/areas_data.js', {}, function(data){
            if(data) {
                _capable = true;
